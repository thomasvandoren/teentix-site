...
class Tag_ext extends Extension_builder_tag
{
	...
	function publish_form_new_tabs( $publish_tabs, $channel_id, $entry_id )
	{
	...
	function publish_form_new_tabs_block( $channel_id )
	{
		...
		if ( ee()->input->get_post('tag_f') !== FALSE AND ee()->input->get_post('tag_f') != '')
		{
		...
		$this->cached_vars['method_base'] = $method_base = BASE . '&C=modules&M=' . $this->lower_name;
		...
		ee()->cp->extra_header .= "\n".'<script type="text/javascript" src="'.$method_base."&method=tag_publish_javascript&ajax=solspace_tag_module&".$this->sc->db->channel_id.'='.$channel_id.'"></script>';
		ee()->cp->extra_header .= "\n".'<link rel="stylesheet" type="text/css" href="'.$method_base.'&method=tag_publish_css&ajax=solspace_tag_module"; media="screen"/>';
	...
	function gallery_cp_entry_form_add_row( $entry_id, $r )
	{
	...
	function gallery_extended_cp_entry_form_add_row( $r )
	{
		ee()->extensions->end_script = FALSE;
		...
		$query	= ee()->db->query( "SELECT COUNT(*) AS count FROM exp_extensions
								   ...
								   AND method = 'gallery_cp_entry_form_add_row'
								   AND enabled = 'y'" );
	...
	{
		return $this->parse($entry_id, $data, $ping_message);
	}

	public function entry_submission_end($entry_id, $data, $ping_message)
	{
		return $this->parse($entry_id, $data, $ping_message);
	}
	...
	function ajax($incoming) { return $this->sessions_end($incoming); }

	function sessions_end( $incoming )
	{
		if ( REQ != 'CP' OR
	...
	function gallery_cp_insert_entry_end( $entry_id = '' ) { $this->gallery_parse( $entry_id ); }

	function gallery_cp_update_entry_end( $entry_id = '' ) { $this->gallery_parse( $entry_id ); }
	...
	function gallery_extended_update_entry_end( $entry_id )
	{
		ee()->extensions->end_script	= FALSE;
