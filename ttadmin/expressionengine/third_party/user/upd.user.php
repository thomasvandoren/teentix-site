...
EOF;
	...
	public function uninstall()
	{
		...
		$this->EE->db->query("ALTER TABLE `exp_members` DROP `profile_views`");
	...
	{
		// --------------------------------------------
		//  ExpressionEngine 2.x attempts to
		//  do automatic updates.
		//	- Mitchell questioned clients/customers and
		//	discovered that the majority preferred to update
		...
		{
			$this->EE->db->query("ALTER TABLE `exp_user_keys` ADD INDEX (`author_id`)");
			$this->EE->db->query("ALTER TABLE `exp_user_keys` ADD INDEX (`member_id`)");
			$this->EE->db->query("ALTER TABLE `exp_user_keys` ADD INDEX (`group_id`)");
			$this->EE->db->query("ALTER TABLE `exp_user_params` ADD INDEX (`hash`)");
		}
		...
		if ($this->version_compare($this->database_version(), '<', '3.0.0.d25'))
		{
			...
			{
				if ($this->EE->config->item($pref) !== FALSE)
				{
					$query = $this->EE->db
					...
					if ($query->num_rows() == 0)
					{
						$this->EE->db->insert(
							...
							array(
								...
								'preference_value'	=> $this->EE->config->item($pref)
							)
						);
					}
					...
					{
						$this->EE->config->_update_config(array(), array($pref));
					}
				}
			}
		}
		...
		if ($this->version_compare($this->database_version(), '<', '3.3.2'))
		{
			...
			$tab_channel_ids = $this->EE->user_model->get_channel_ids(FALSE);
	...
	public function remove_user_tabs()
	{
		...
		$this->EE->layout->delete_layout_tabs(
			array_merge_recursive($this->old_layout_data, $this->tabs())
		);

		$this->EE->layout->delete_layout_fields(
			array_merge_recursive($this->old_layout_data, $this->tabs())
		);
	}
