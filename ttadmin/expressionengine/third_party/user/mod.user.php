...
class User extends Module_builder_user
{
	...
	public function __construct()
	{
		...
		if ($this->EE->config->item('req_mbr_activation') == 'manual' AND
			$this->EE->db->table_exists('exp_user_welcome_email_list'))
		{
	...
	public function stats()
	{
		...
		{
			$exclude	= array_merge( $exclude, explode( "|", $this->EE->TMPL->fetch_param('exclude') ) );
		}
		...
		{
			$include	= array_merge( $include, explode( "|", $this->EE->TMPL->fetch_param('include') ) );
		}
		...
		$query	= $this->EE->db->query(
			"SELECT m.*, mg.group_title,
			 ...
			 WHERE 	member_id = '".$this->EE->db->escape_str( $this->member_id )."'
			 AND 	m.group_id = mg.group_id
			 AND 	mg.site_id = '".$this->EE->db->escape_str($this->EE->config->slash_item('site_id'))."'"
		);
		...
		$query_row['photo_url']				= $this->EE->config->item('photo_url');
		$query_row['avatar_url']			= $this->EE->config->slash_item('avatar_url');
		$query_row['sig_img_url']			= $this->EE->config->slash_item('sig_img_url');
		...
		if ( ! isset($query_row['total_ratings']))
		{
			...
			if (stristr($this->EE->TMPL->tagdata, 'total_ratings') && $this->EE->db->table_exists('exp_rating_stats'))
			{
				$rquery = $this->EE->db->query("SELECT count FROM exp_rating_stats
										   WHERE member_id = '".$this->EE->db->escape_str( $this->member_id )."'");
		...
		$this->member_only					= ( $this->EE->TMPL->fetch_param('member_only') !== FALSE AND
												$this->EE->TMPL->fetch_param('member_only') == 'no' ) ? FALSE: TRUE;
		...
		if ($query->row('bio') != '')
		{
			$query_row['bio'] = $this->EE->typography->parse_type(
				$query->row('bio'),
				array(
					'text_format'   => 'xhtml',
					'html_format'   => 'safe',
		...
		{
			$query_row['signature'] = $this->EE->typography->parse_type(
				$query->row('signature'),
				array(
					'text_format'   => 'xhtml',
					'html_format'   => 'safe',
					'auto_links'    => 'y',
					'allow_img_url' => ($this->EE->config->item('sig_allow_img_hotlink') == 'y') ? 'y' : 'n'
				)
			);
		}
		...
		if ( preg_match( "/".LD.'all'.RD."(.*?)".LD.preg_quote(T_SLASH, '/').'all'.RD."/s", $tagdata, $match ) )
		{
			...
			foreach ( $query->row_array() as $key => $val )
			{
				...
				{
					$all	= str_replace( LD.'value'.RD, $this->EE->localize->set_human_time($val), $all );
				}
				else
				{
		...
		if ( $custom->num_rows() > 0 )
		{
			foreach ( $this->_mfields() as $key => $val )
			{
				...
				foreach ( $this->EE->TMPL->var_pair as $k => $v )
				{
					if ( $k == "select_".$key )
					{
						$data		= $this->EE->TMPL->fetch_data_between_var_pairs( $tagdata, $k );
	...
	public function users()
	{
		...
		if ( $this->EE->TMPL->fetch_param('disable') !== FALSE AND stristr('member_data', $this->EE->TMPL->fetch_param('disable')))
		{
		...
		if (stristr($this->EE->TMPL->tagdata, LD.'group_title'.RD) OR stristr($this->EE->TMPL->tagdata, LD.'group_description'.RD))
		{
		...
		if ( $this->EE->TMPL->fetch_param('dynamic') !== FALSE AND $this->check_no($this->EE->TMPL->fetch_param('dynamic')))
		{
			$dynamic	= FALSE;
		}
		...
		if ( $this->EE->TMPL->fetch_param('category') !== FALSE AND $this->EE->TMPL->fetch_param('category') != '' )
		{
			$category	= str_replace( "C", "", $this->EE->TMPL->fetch_param('category') );
		}
		elseif ( preg_match("/\/C(\d+)/s", $this->EE->uri->uri_string, $match ) AND $dynamic === TRUE )
		{
			$category	= $match['1'];
		}
		...
		if ($this->EE->uri->uri_string != '' AND $this->EE->config->item("reserved_category_word") != '' AND in_array($this->EE->config->item("reserved_category_word"), explode("/", $this->EE->uri->uri_string)) AND $dynamic)
		{
			if (preg_match("/(^|\/)".preg_quote($this->EE->config->item("reserved_category_word"))."\/(.+?)($|\/)/i", $this->EE->uri->uri_string, $cmatch))
			{
				$cquery = $this->EE->db->query("SELECT cat_id
											FROM exp_categories
											WHERE site_id IN ('".implode("','", $this->EE->db->escape_str($this->EE->TMPL->site_ids))."') AND
											cat_url_title = '".$this->EE->db->escape_str($cmatch[2])."'");
		...
		if (stristr($this->EE->TMPL->tagdata, LD.'group_title'.RD) OR stristr($this->EE->TMPL->tagdata, LD.'group_description'.RD))
		{
			$sql .= " AND mg.site_id = '".$this->EE->db->escape_str($this->EE->config->item('site_id'))."'";
		}
		...
		if ( isset( $category ) === TRUE )
		{
			$sql .= " AND ucp.cat_id = '".$this->EE->db->escape_str( $category )."'";
		}
		...
		if ( $member_ids = $this->EE->TMPL->fetch_param('member_id') )
		{
			$sql .= $this->EE->functions->sql_andor_string( $member_ids, 'm.member_id' );
		}
		...
		if ( $this->EE->TMPL->fetch_param('alpha') !== FALSE AND $this->EE->TMPL->fetch_param('alpha') != '' )
		{
			$alpha	= $this->EE->TMPL->fetch_param('alpha');

			$sql   .= " AND m.screen_name LIKE '".$this->EE->db->escape_like_str( $alpha )."%'";
		}
		...
		$filter_method = (in_array($this->EE->TMPL->fetch_param('filter_method'), $possible_filters)) ? $this->EE->TMPL->fetch_param('filter_method') : 'any';

		/**	----------------------------------------
		/**	Filter by screen name
		/**	----------------------------------------*/

		if ( $letter = $this->EE->TMPL->fetch_param('screen_name') )
		{
			if ( $filter_method == 'exact' )
			{
				$sql .= " AND m.screen_name = '".$this->EE->db->escape_str($letter)."'";
			}
			elseif($filter_method == 'any')
			{
				$sql .= " AND m.screen_name LIKE '%".$this->EE->db->escape_like_str($letter)."%'";
			}
		}
		...
		if ( $standard_member_field = $this->EE->TMPL->fetch_param('standard_member_field') )
		{
			...
			if ( isset( $standard_field[0] ) AND isset( $standard_field[1] ) AND in_array( $standard_field[0], $this->standard))
			{
				...
				{
					$sql .= " AND `m`.`".$standard_field[0]."` = '".$this->EE->db->escape_str($standard_field[1])."'";
				}
				else
				{
					$sql .= " AND `m`.`".$standard_field[0]."` LIKE '%".$this->EE->db->escape_like_str($standard_field[1])."%'";
				}
			}
		}
		...
		if ( $custom_member_field = $this->EE->TMPL->fetch_param('custom_member_field') )
		{
			$this->_mfields();
			...
			if ( isset( $custom_field[0] ) AND isset( $custom_field[1] ) AND isset( $this->mfields[ $custom_field[0] ] ) )
			{
				...
				elseif ( $filter_method == 'exact' )
				{
					$sql .= " AND `md`.`m_field_id_".$this->mfields[ $custom_field[0] ]['id']."` = '".$this->EE->db->escape_str($custom_field[1])."'";
				}
				else
				{
					$sql .= " AND `md`.`m_field_id_".$this->mfields[ $custom_field[0] ]['id']."` LIKE '%".$this->EE->db->escape_like_str($custom_field[1])."%'";
				}
			}
		}
		...
		} // End is_array($this->EE->TMPL->tagparams)
		...
		$use_prefix = stristr($this->EE->TMPL->tagdata, LD . 'user_paginate' . RD);
		...
		$this->query	= $this->EE->db->query( $sql );
		...
		$this->EE->TMPL->log_item('Query Processed and Done.');
	...
	public function _users( $inject = array() )
	{
		...
		$photo_url		= $this->EE->config->slash_item('photo_url');
		$avatar_url		= $this->EE->config->slash_item('avatar_url');
		$sig_img_url	= $this->EE->config->slash_item('sig_img_url');
		...
		if ($this->EE->extensions->active_hook('user_users_start') === TRUE)
		{
			$inject = $this->EE->extensions->universal_call( 'user_users_start', $query_result, $inject );
		...
		if ( $this->EE->TMPL->fetch_param('orderby') !== FALSE AND
			 in_array($this->EE->TMPL->fetch_param('orderby') ,array("primary", "principal")) )
		{
			usort($query_result, array(&$this, "principal_sort"));

			if ( $this->EE->TMPL->fetch_param('sort') !== FALSE AND $this->EE->TMPL->fetch_param('sort') == "desc" )
			{
		...
		foreach ($dates as $val)
		{
			...
			if (preg_match_all("/".LD.$val."\s+format=([\"'])([^\\1]*?)\\1".RD."/s", $this->EE->TMPL->tagdata, $matches))
			{
				...
				for ($j = 0; $j < count($matches[0]); $j++)
				{
					...
					$date_vars[$val][$matches[0][$j]] = $this->EE->localize->fetch_date_params($matches[2][$j]);
				}
			}
		}
		...
		$this->EE->TMPL->log_item('Beginning Query Result Loop');
		foreach ( $query_result as $count => $row )
		{
			...
			foreach ($this->EE->TMPL->var_single as $key => $val)
			{
				...
				{
					$row['bio'] = $this->EE->typography->parse_type(
						$row['bio'],
						array(
							'text_format'   => 'xhtml',
							'html_format'   => 'safe',
				...
				{
					$row['signature'] = $this->EE->typography->parse_type(
						$row['signature'],
						array(
							'text_format'   => 'xhtml',
							'html_format'   => 'safe',
				...
				foreach($date_vars as $type => $matches)
				{
					...
					if (APP_VER < 2.0)
					{
						foreach ( $matches[$key] as $code )
						{
							$val	= str_replace(
									$code,
									$this->EE->localize->convert_timestamp($code, $row[$type], TRUE ),
									$val);
						}
					}
					else
			...
			foreach ( $this->mfields as $key => $val )
			{
				...
				if (isset($this->EE->TMPL->var_pair["select_".$key]))
				{
				...
				if (isset($this->EE->TMPL->var_single['abbr_'.$key]))
				{
					if (empty($row['m_field_id_'.$val['id']]))
					{
						$tagdata = $this->EE->TMPL->swap_var_single('abbr_'.$key, '', $tagdata);
						continue;
					}
				...
				if (isset($this->EE->TMPL->var_single[$key]))
				{
					if (empty($row['m_field_id_'.$val['id']]))
					{
						$tagdata = $this->EE->TMPL->swap_var_single($key, '', $tagdata);
		...
		if ( isset($this->EE->TMPL) AND is_object($this->EE->TMPL) AND ctype_digit( $this->EE->TMPL->fetch_param('backspace') ) )
		{
	...
	public function authors()
	{
		...
		if ( $this->EE->db->table_exists( 'exp_user_authors' ) === FALSE )
		{
			return $this->no_results('user');
		}
		...
		$sql	= "SELECT ua.author_id, ua.principal
				   ...
				   WHERE ua.entry_id = '".$this->EE->db->escape_str( $this->entry_id )."'";
		...
		foreach ( $query->result_array() as $row )
		{
			...
			$ids[]	= $this->EE->db->escape_str( $row['author_id'] );
		}

		//why do you hate me kelsey?
		if (is_object($this->EE->TMPL) AND
			isset($this->EE->TMPL->tagparams['orderby']) AND
			$this->EE->TMPL->tagparams['orderby'] === 'primary')
		{
			$this->EE->TMPL->tagparams['orderby'] = 'principal';
		}
		...
		$sql	.= " WHERE ua.author_id IN ('".implode( "','", $ids )."')
					 AND ua.entry_id = '".$this->EE->db->escape_str( $this->entry_id )."'";
	...
	public function entries()
	{
		...
		$query	= $this->EE->db->query(
			"SELECT DISTINCT 	entry_id
			 ...
			 WHERE 				author_id != 0
			 ".$this->EE->functions->sql_andor_string( $this->member_id, 'author_id'));
	...
	private function _entries ( $params = array() )
	{
		...
		$this->EE->TMPL->tagparams['entry_id']	= $this->entry_id;
		...
		if ( preg_match("#(/P\d+)#", $this->EE->uri->uri_string, $match) )
		{
			$channel->return_data	= str_replace( $match['1'], "", $channel->return_data );
		}
		elseif ( preg_match("#(P\d+)#", $this->EE->uri->uri_string, $match) )
		{
			$channel->return_data	= str_replace( $match['1'], "", $channel->return_data );
		}
	...
	public function edit()
	{
		...
		if ($this->EE->session->userdata['member_id'] != $this->member_id AND
			$this->EE->session->userdata['group_id'] != 1
			AND
			(
				$this->EE->TMPL->fetch_param('group_id') === FALSE OR
				(
					$this->EE->TMPL->fetch_param('group_id') !== FALSE AND
					! in_array(
						$this->EE->session->userdata['group_id'],
						preg_split(
							"/,|\|/",
							$this->EE->TMPL->fetch_param('group_id')
						)
					)
				)
			)
		...
		if ($this->EE->extensions->active_hook('user_edit_start') === TRUE)
		{
			$edata = $this->EE->extensions->universal_call(
				'user_edit_start',
				$this->member_id
			);
			if ($this->EE->extensions->end_script === TRUE) return;
		...
		$query_row['photo_url']		= $this->EE->config->slash_item('photo_url');
		$query_row['avatar_url']	= $this->EE->config->slash_item('avatar_url');
		$query_row['sig_img_url']	= $this->EE->config->slash_item('sig_img_url');
		...
		foreach ( $this->EE->TMPL->var_pair as $key => $val )
		{
			...
			{
				if ($this->EE->TMPL->fetch_param('allowed_groups') !== FALSE)
				{
					$data = $this->EE->TMPL->fetch_data_between_var_pairs(
						$tagdata,
						$key
					);
					...
					$tagdata	= preg_replace(
						"/" . LD . $key . RD .
							"(.*?)" .
						LD . preg_quote(T_SLASH, '/') . $key . RD."/s",
						str_replace(
							'$',
							'\$',
			...
			if ( $key == 'timezones' )
			{
				...
				foreach ( $this->EE->localize->zones() as $key => $val )
				{
					$out		= $match['1'];
		...
		$this->form_data['ACT']		= $this->EE->functions->fetch_action_id(
			'User',
			'edit_profile'
		);
		...
		$this->form_data['RET']		= (
			isset($_POST['RET'])
		) ? $_POST['RET'] : $this->EE->functions->fetch_current_uri();

		if ( $this->EE->TMPL->fetch_param('form_name') !== FALSE AND
			$this->EE->TMPL->fetch_param('form_name') != '' )
		{
			$this->form_data['name'] = $this->EE->TMPL->fetch_param('form_name');
		}

		$this->form_data['id']		= $this->EE->TMPL->fetch_param(
			'form_id',
			'member_form'
		);

		$this->form_data['return']	= $this->EE->TMPL->fetch_param(
			'return',
			''
		);
		...
		$params = array(
			...
			'username_override'		=> $this->EE->TMPL->fetch_param(
				'username_override',
				''
			),
			'exclude_username'		=> $this->EE->TMPL->fetch_param(
				'exclude_username',
				''
			),
			'required'				=> $this->EE->TMPL->fetch_param(
				'required',
				''
			),
			'group_id'				=> $this->EE->TMPL->fetch_param(
				'group_id',
				''
			),
			'password_required'		=> $this->EE->TMPL->fetch_param(
				'password_required',
				''
			),
			'secure_action'			=> $this->EE->TMPL->fetch_param(
				'secure_action',
				'no'
			),
			'screen_name_password_required' => $this->EE->TMPL->fetch_param(
		...
		if ($this->EE->TMPL->fetch_param('allowed_groups') !== FALSE AND
			$this->EE->TMPL->fetch_param('allowed_groups') != '')
		{
			$this->params['allowed_groups']	= $this->EE->TMPL->fetch_param(
				'allowed_groups'
			);
		}
	...
	public function edit_profile()
	{
		...
		if ( $this->EE->session->userdata['member_id'] != $member_id AND
			$admin === FALSE )
		{
			return $this->_output_error(
				'general',
		...
		if ($wquery->num_rows() >= 2)
		{
			...
			$oquery   = $this->EE->db->query(
				"SELECT m.*, md.*
				 FROM 	exp_members AS m,
						exp_member_data AS md
				 WHERE 	md.member_id = m.member_id
				 AND 	m.member_id = '".$this->EE->db->escape_str($member_id)."'"
			);
		...
		if ($this->EE->input->post('username') AND
			$this->EE->session->ban_check(
				'username',
		...
		if ($this->EE->input->get_post('screen_name') === FALSE AND
		...
		elseif ( trim($this->EE->input->get_post('screen_name')) == '' )
		{
			$_POST['screen_name']	= $this->EE->input->post('username');
		}
		...
		$validate	= array(
			...
			'screen_name'		=> stripslashes($this->EE->input->post('screen_name')),
		...
		{
			$validate['username']	= $this->EE->input->post('username');
		}

		if ( $this->EE->input->get_post('email') )
		{
			$validate['email']	= $this->EE->input->get_post('email');
		}

		if ( $this->EE->input->get_post('password') )
		{
			$validate['password']	= $this->EE->input->get_post('password');
		}

		if ( $this->EE->input->get_post('password_confirm') )
		{
			$validate['password_confirm']	= $this->EE->input->get_post('password_confirm');
		}

		if ( $this->EE->input->get_post('current_password') )
		{
			$validate['cur_password']	= $this->EE->input->get_post('current_password');
		}
		...
		if ($this->EE->input->get_post('password') !== FALSE AND
			$this->EE->input->get_post('password') != '' AND
			$this->EE->input->get_post('current_password') !=
				$this->EE->input->get_post('password') AND
			$member_id == $this->EE->session->userdata['member_id'])
		{
			...
			$this->check_yes($this->_param('password_required')))
		{
			...
			if ($this->EE->input->get_post('current_password') === FALSE OR
				$this->EE->input->get_post('current_password') == '')
			{
				return $this->_output_error('general', array(lang('invalid_password')));
			}

			$passwd = $this->EE->auth->hash_password(
				$this->EE->input->get_post('current_password'),
				$query->row('salt')
			);
		...
		if ($admin === FALSE)
		{
			...
			{
				if ($this->EE->input->get_post($val) !== FALSE AND
					$this->EE->input->get_post($val) != $query->row($val))
				{
					if ($this->EE->input->get_post('current_password') === FALSE OR
						$this->EE->input->get_post('current_password') == '')
					{
		...
		if ( $this->EE->input->post('language') !== FALSE AND
			 $this->EE->input->post('language') != '' )
		{
			$language = $this->EE->security->sanitize_filename(
				$this->EE->input->get_post('language')
			);
		...
		if ( $this->_param('required') !== FALSE)
		{
			...
			foreach ( $required as $req )
			{
				if ( $req == 'all_required')
				{
					foreach ( $this->mfields as $key => $val )
					{
						if ( ! $this->EE->input->get_post($key) AND $val['required'] == 'y' )
						{
							$missing[]	= $this->mfields[$key]['label'];
						}
					}
				}
				elseif ( ! $this->EE->input->get_post($req) )
				{
		...
		foreach ( $this->_mfields() as $key => $val )
		{
			if ( $val['required'] == 'y' AND ! $this->EE->input->get_post($key) )
			{
		...
		if ( isset( $_POST['username'] ) )
		{
			if ( $this->EE->input->post('username') != '' )
			{
				if ( $this->EE->input->post('username') != $query->row('username') )
				{
					if ( $this->EE->config->item('allow_username_change') == 'y' )
					{
							...
							)) !== FALSE)
						{
							...
							{
								$this->EE->validate->errors[$key] = str_replace(
									'32',
									'50',
									$this->EE->validate->errors[$key]
								);
							}
						}
					}
			...
			{
				$this->EE->validate->errors[] = lang('missing_username');
			}
		}
		...
		if ($this->EE->input->get_post('password') AND
		...
		if ($this->EE->extensions->active_hook('user_edit_validate') === TRUE)
		{
			...
			{
				$this->EE->validate->errors = array_merge(
					$this->EE->validate->errors,
					$errors
				);
			}
		...
		if ($query->row('screen_name') != $_POST['screen_name'] AND
			$this->EE->config->item('forum_is_installed') == "y" )
		{
			$this->EE->db->update(
				'forums',
		...
		if ( $this->EE->input->post('screen_name') )
		{
			$this->insert_data['screen_name'] = $this->EE->input->post('screen_name');
		}

		if ( $this->EE->input->post('username') AND
			 $this->EE->config->item('allow_username_change') == 'y')
		{
			$this->insert_data['username'] = $this->EE->input->get_post('username');
		}
		...
		if ($this->EE->input->get_post('password'))
		{
			$this->EE->auth->update_password(
				$member_id,
				stripslashes($this->EE->input->get_post('password'))
			);
		}
		...
		if ( $this->EE->input->get_post('email') )
		{
			$this->insert_data['email'] = $this->EE->input->get_post('email');
		}
		...
		if ( $this->_param('checks') != '' )
		{
			foreach ( explode( "|", $this->_param('checks') )  as $c )
			{
				...
				{
					if ( $this->EE->input->post($c) !== FALSE )
					{
						if ( stristr( $this->EE->input->post($c), 'n' ) )
						{
		...
		if ( $this->EE->input->post('group_id') AND
			 $this->EE->input->post('group_id') != $query->row('group_id') AND
			 $this->EE->session->userdata('group_id') == '1' )
		{
			...
			$this->insert_data['group_id']	= $this->EE->db->escape_str(
				$this->EE->input->post('group_id')
			);
		}
		elseif($this->EE->input->post('group_id') !== FALSE AND
			ctype_digit($this->EE->input->post('group_id')) AND
			$this->_param('allowed_groups') !== FALSE)
		{
			$sql = "SELECT DISTINCT group_id
					FROM exp_member_groups
					WHERE group_id NOT IN (1,2,3,4)
					AND group_id = '".$this->EE->db->escape_str(
						$this->EE->input->post('group_id')
					)."' ".
					$this->EE->functions->sql_andor_string(
						$this->_param('allowed_groups'),
						'group_id'
					);

			$gquery = $this->EE->db->query($sql);
		...
		/*
		elseif (
			$this->EE->input->post('group_id') !== FALSE AND
			...
			ctype_digit( $this->EE->input->post('group_id') ) )
		{
			$this->insert_data['group_id']	= $this->EE->input->post('group_id');
		}
		*/
		else
		{
		...
		{
			$this->insert_data['last_activity'] = $this->EE->localize->now;
		}
		...
		if ($this->EE->extensions->active_hook('user_edit_insert_data') === TRUE)
		{
			$this->insert_data = $this->EE->extensions->universal_call(
				'user_edit_insert_data',
				$member_id,
				$this->insert_data,
				$this
		...
		if ($this->EE->extensions->active_hook('user_edit_end') === TRUE)
		{
			$edata = $this->EE->extensions->universal_call(
				'user_edit_end',
				$query->row('member_id'),
		...
		{
			$this->EE->functions->redirect($this->_param('override_return'));
			exit();
		}
		...
		if ( $this->EE->input->post('username') !== FALSE AND
			 $old_username != $this->EE->input->post('username') )
		{
			if (stristr( $return, "/user/" . $old_username))
			{
				$return	= str_replace(
					"/user/" . $old_username,
					"/user/" . $this->EE->input->get_post('username'),
					$return
				);
			}
		}
		...
		if ($changing_password)
		{
			...
			return $this->EE->output->show_message(array(
				'title'		=> lang('success'),
		...
		$this->EE->functions->redirect($this->_chars_decode($return));
		exit();
	}
	// END edit profile
	...
	private function _categories( $tagdata, $only_show_selected = '' )
	{
		...
		if ( isset($this->EE->TMPL)  AND
			 $this->EE->TMPL->fetch_param('parent_id') !== FALSE AND
			 ctype_digit( $this->EE->TMPL->fetch_param('parent_id') ) )
		{
		...
		if (count($this->cat_chunk) == 0 AND
			preg_match_all(
				"/".LD."categories(.*?)".RD."(.*?)".
					LD.preg_quote(T_SLASH, '/')."categories".RD."/s",
				$tagdata,
				$matches
			) )
		{
			for ($j = 0; $j < count($matches['0']); $j++)
			{
				...
				) as $val )
				{
					if ( preg_match(
							"/".LD.$val.RD."(.*?)".
							LD.preg_quote(T_SLASH, '/').$val.RD."/s",
	...
	private function _fetch_category_parents(array $cat_array = array())
	{
		...
		{
			$sql	.= " AND site_id = '".$this->EE->db->escape_str($this->EE->config->item('site_id'))."'";
		}

		$sql	.= " AND cat_id IN ('".implode("','", $this->EE->db->escape_str($cat_array))."')";
	...
	public function _category_group_vars( $group_id = 0, $data = '' )
	{
		...
		{
			$data   = $this->EE->functions->prep_conditionals(
				$data,
				array(
		...
		if ( $this->cat_params['group_id'] != '' )
		{
			$sql	.= $this->EE->functions->sql_andor_string( $this->cat_params['group_id'], 'group_id' );
		}
	...
	public function groups()
	{
		...
		if ( $this->EE->session->userdata['group_id'] == 1 OR
			( $this->EE->TMPL->fetch_param('authorized_group') !== FALSE AND
			  ctype_digit( preg_split( "/,|\|/", $this->EE->TMPL->fetch_param('authorized_group'), -1, PREG_SPLIT_NO_EMPTY ) ) &&
			  in_array( $this->EE->session->userdata['group_id'], preg_split( "/,|\|/", $this->EE->TMPL->fetch_param('authorized_group'), -1, PREG_SPLIT_NO_EMPTY ) ) !== FALSE
			)
		   )
		{
		}
		...
		if ( $this->EE->TMPL->fetch_param('editable_group') !== FALSE AND $this->EE->TMPL->fetch_param('editable_group') != '' )
		{
			if ( $this->EE->TMPL->fetch_param('editable_group') == 'all' )
			{
				$all	= TRUE;
			}
			else
			{
				$editable_group		= preg_split( "/,|\|/", $this->EE->TMPL->fetch_param('editable_group'), -1, PREG_SPLIT_NO_EMPTY );
			}
		...
		if ( $all === TRUE AND preg_match( "/".LD."group".RD."(.*?)".LD.preg_quote(T_SLASH, '/')."group".RD."/s", $tagdata, $match ) > 0 )
		{
			...
			foreach ( $query->result_array() as $row )
			{
				...
				$tdata	= $this->EE->functions->prep_conditionals( $tdata, $cond );
				...
				{
					$tdata	= $this->EE->TMPL->swap_var_single( $key, $val, $tdata );
				}
				...
				foreach ( $this->_mfields() as $key => $val )
				{
					...
					foreach ( $this->EE->TMPL->var_pair as $k => $v )
					{
						if ( $k == "select_".$key )
						{
							$data		= $this->EE->TMPL->fetch_data_between_var_pairs( $tdata, $k );
					...
					$tdata	= $this->EE->TMPL->swap_var_single( $key, $row['m_field_id_'.$val['id']], $tdata );
				}
		...
		$this->form_data['ACT']						= $this->EE->functions->fetch_action_id('User', 'group_edit');

		$this->form_data['RET']						= (isset($_POST['RET'])) ? $_POST['RET'] : $this->EE->functions->fetch_current_uri();

		if ( $this->EE->TMPL->fetch_param('form_name') !== FALSE AND $this->EE->TMPL->fetch_param('form_name') != '' )
		{
			$this->form_data['name']	= $this->EE->TMPL->fetch_param('form_name');
		}

		$this->form_data['id']						= ( $this->EE->TMPL->fetch_param('form_id') !== FALSE ) ? $this->EE->TMPL->fetch_param('form_id'): 'member_form';

		$this->form_data['return']					= ( $this->EE->TMPL->fetch_param('return') !== FALSE ) ? $this->EE->TMPL->fetch_param('return'): '';

		$this->params['checks']					= $checks;

		$this->params['custom_checks']			= $custom_checks;

		$this->params['required']				= ( $this->EE->TMPL->fetch_param('required') !== FALSE ) ? $this->EE->TMPL->fetch_param('required'): '';

		$this->params['authorized_group']		= ( $this->EE->TMPL->fetch_param('authorized_group') !== FALSE ) ? $this->EE->TMPL->fetch_param('authorized_group'): '';

		$this->params['editable_group']			= ( $this->EE->TMPL->fetch_param('editable_group') !== FALSE ) ? $this->EE->TMPL->fetch_param('editable_group'): '';

		$this->params['secure_action']			= ( $this->EE->TMPL->fetch_param('secure_action') !== FALSE) ? $this->EE->TMPL->fetch_param('secure_action'): 'no';
	...
	public function group_edit()
	{
		...
		if ($this->EE->blacklist->blacklisted == 'y' AND $this->EE->blacklist->whitelisted == 'n')
		{
			return $this->_output_error('general', array(lang('not_authorized')));
		}
		...
		if ( $this->EE->session->userdata['group_id'] == 1 OR
			 ( $this->_param('authorized_group') !== FALSE &&
			   ...
			   in_array( $this->EE->session->userdata['group_id'], preg_split( "/,|\|/", $this->_param('authorized_group'), -1, PREG_SPLIT_NO_EMPTY ) ) !== FALSE ) )
		{
		}
		else
		{
			...
			if ( is_array( $val ) === TRUE )
			{
				...
				if ( in_array( $key, $this->standard ) === TRUE OR in_array( $key, array('group_id') ) === TRUE OR isset( $this->mfields[$key] ) === TRUE )
				{
					...
					foreach ( $val as $k => $v )
					{
						if ( ctype_digit( $k ) )
						{
							$members[$k][$key]	= $this->EE->security->xss_clean( $v );
						}
					}
				}
			}
		...
		if ( $all === FALSE )
		{
			...
			foreach ( $editable_group as $group )
			{
				$sql	.= "'".$this->EE->db->escape_str( $group )."',";
			}
		...
		{
			$sql	.= "'".$this->EE->db->escape_str( $member )."',";
		}
		...
		foreach ( $query->result_array() as $row )
		{
			...
			if ( ! empty( $data ) )
			{
				$this->EE->db->query( $this->EE->db->update_string( 'exp_members', $data, array( 'member_id' => $row['member_id'] ) ) );
			}
			...
			if ( ! empty( $cfields ) )
			{
				$this->EE->db->query( $this->EE->db->update_string( 'exp_member_data', $cfields, array( 'member_id' => $row['member_id'] ) ) );
			}
			...
			*/
			if ($this->EE->extensions->active_hook('user_group_edit_end') === TRUE)
			{
				$edata = $this->EE->extensions->universal_call(
					'user_group_edit_end',
					$row['member_id'],
		...
		if ( $this->EE->input->get_post('return') !== FALSE AND $this->EE->input->get_post('return') != '' )
		{
			$return	= $this->EE->input->get_post('return');
		}
		elseif ( $this->EE->input->get_post('RET') !== FALSE AND $this->EE->input->get_post('RET') != '' )
		{
			$return	= $this->EE->input->get_post('RET');
		}
		else
	...
	public function register()
	{
		...
		if ( $this->EE->session->userdata('member_id') != 0 AND
			 $this->EE->TMPL->fetch_param('admin_register') !== 'yes')
		{
			if ($this->EE->TMPL->fetch_param('admin_register') !== 'yes' OR
			   ($this->EE->session->userdata['group_id'] != 1 AND
				$this->EE->session->userdata['can_admin_members'] !== 'y'))
			{
		...
		if ( preg_match( "#/".self::$key_trigger."/(\w+)/?#", $this->EE->uri->uri_string, $match ) )
		{
			$tagdata		= $this->EE->TMPL->swap_var_single( 'key', $match['1'], $tagdata );
		}
		else
		{
			$tagdata		= $this->EE->TMPL->swap_var_single( 'key', '', $tagdata );
		...
		foreach ( $this->EE->TMPL->var_pair as $key => $val )
		{
			...
			if ($key == 'select_member_groups')
			{
				if ($this->EE->TMPL->fetch_param('allowed_groups') !== FALSE)
				{
					$data		= $this->EE->TMPL->fetch_data_between_var_pairs( $tagdata, $key );
		...
		if ( $this->EE->TMPL->fetch_param( 'no_form' ) == "yes" )
		{
			return $tagdata;
		}
		...
		$this->form_data['ACT']						= $this->EE->functions->fetch_action_id('User', 'reg');

		if (isset($_POST['RET']))
		{
			 $this->form_data['RET'] = $_POST['RET'];
		}
		elseif($this->EE->TMPL->fetch_param('return') !== FALSE)
		{
			$this->form_data['RET'] = $this->EE->TMPL->fetch_param('return');
		}
		else
		{
			$this->form_data['RET'] = $this->EE->functions->fetch_current_uri();
		}
		...
		$this->params['group_id']				= ( $this->EE->TMPL->fetch_param('group_id') !== FALSE ) ? $this->EE->TMPL->fetch_param('group_id'): '';

		$this->params['notify']					= ( $this->EE->TMPL->fetch_param('notify') !== FALSE ) ? $this->EE->TMPL->fetch_param('notify'): '';

		$this->params['screen_name_override']	= ( $this->EE->TMPL->fetch_param('screen_name') !== FALSE ) ? $this->EE->TMPL->fetch_param('screen_name'): '';

		$this->params['exclude_username']		= ( $this->EE->TMPL->fetch_param('exclude_username') ) ? $this->EE->TMPL->fetch_param('exclude_username'): '';

		$this->params['require_key']			= ( $this->EE->TMPL->fetch_param('require_key') ) ? $this->EE->TMPL->fetch_param('require_key'): '';

		$this->params['key_email_match']		= ( $this->EE->TMPL->fetch_param('key_email_match') ) ? $this->EE->TMPL->fetch_param('key_email_match'): '';

		$this->params['key']					= ( $this->EE->TMPL->fetch_param('key') != '' ) ? $this->EE->TMPL->fetch_param('key'): '';

		$this->params['secure_action']			= ( $this->EE->TMPL->fetch_param('secure_action') !== FALSE) ? $this->EE->TMPL->fetch_param('secure_action'): 'no';

		$this->params['admin_register']			= ( $this->EE->TMPL->fetch_param('admin_register') !== FALSE) ? $this->EE->TMPL->fetch_param('admin_register'): 'no';

		$this->params['required']				= ( $this->EE->TMPL->fetch_param('required') ) ? $this->EE->TMPL->fetch_param('required'): '';

		if ($this->EE->TMPL->fetch_param('allowed_groups') !== FALSE AND $this->EE->TMPL->fetch_param('allowed_groups') != '')
		{
			$this->params['allowed_groups']		=  $this->EE->TMPL->fetch_param('allowed_groups');
		}
	...
	{
		$this->EE->load->helper('url'); // For prep_url();
		...
		if ($this->EE->extensions->active_hook('user_register_start') === TRUE)
		{
			$edata = $this->EE->extensions->universal_call('user_register_start', $this);
			if ($this->EE->extensions->end_script === TRUE) return;
		}
		...
		{
			$_POST['username']	= $this->EE->input->get_post('email');
		}

		if ( ! $this->EE->input->get_post('screen_name') OR
			 $this->EE->input->get_post('screen_name') == '' )
		{
		...
		if ($this->EE->session->ban_check('username', $_POST['username']))
		{
			return $this->_output_error(
				'general',
				array(lang('prohibited_username'))
		...
		$this->EE->load->library('validate', $validate_config, 'validate');
		...
		if ($this->EE->config->item('use_membership_captcha') == 'y')
		{
			if ($this->EE->config->item('captcha_require_members') == 'y'  OR
				($this->EE->config->item('captcha_require_members') == 'n' AND
				 $this->EE->session->userdata('member_id') == 0)
			 )
			{
				// Hidden configuration!  Disables CAPTCHA on Remote Registrations.
				if ($remote === TRUE && $this->EE->config->item('user_disable_remote_captcha') == 'y')
				{
					// Nothing...
				}
		...
		$errors = array_merge($this->EE->validate->errors, $cust_errors);
	   ...
	   /** --------------------------------------------*/
		...
		if ($this->EE->extensions->active_hook('user_register_error_checking') === TRUE)
		{
			$errors = $this->EE->extensions->universal_call('user_register_error_checking', $this, $errors);
			if ($this->EE->extensions->end_script === TRUE) return;
		}
			 ...
			 $this->_param('key_email_match') == 'yes' )
		{
			...
			$sql = "SELECT 	key_id
					...
					WHERE 	member_id = '0'
					AND 	hash = '" .
					$this->EE->db->escape_str( $this->EE->input->get_post('key') ) . "'";

			if ( $this->_param('key_email_match') == 'yes' )
			{
				$sql	.= " AND email = '" .
							$this->EE->db->escape_str( $this->EE->input->get_post('email') )."'";
			}
		...
		else
		{
			...
			{
				$this->insert_data['group_id'] = $this->EE->config->item('default_member_group');
			}
		}
			 ...
			 $this->_param('group_id') != '1' )
		{
			...
			if ($this->EE->config->item('req_mbr_activation') != 'email' AND
				$this->EE->config->item('req_mbr_activation') != 'manual')
			{
		...
		{
			$key = $this->EE->db->query(
				"SELECT k.group_id
				 FROM 	exp_user_keys AS k
				 JOIN 	exp_member_groups AS g
				 ON 	g.group_id = k.group_id
				 WHERE 	k.key_id = '" . $this->EE->db->escape_str($key_id) . "'
				 AND 	k.group_id NOT IN (0, 1)" );

			if ( $key->num_rows() > 0 )
			{
				if ($this->EE->config->item('req_mbr_activation') == 'email' OR
					$this->EE->config->item('req_mbr_activation') == 'manual')
				{
		...
		if($this->EE->input->post('group_id') !== FALSE AND
		   ctype_digit($this->EE->input->post('group_id')) AND
		   $this->_param('allowed_groups') )
		{
			$sql = "SELECT DISTINCT group_id
					...
					NOT IN 	(1,2,3,4)
					AND 	group_id = '" .
						$this->EE->db->escape_str($this->EE->input->post('group_id')) . "' " .
						$this->EE->functions->sql_andor_string(
							$this->_param('allowed_groups'),
							'group_id'
						);

			$mquery = $this->EE->db->query($sql);
		...
		$query = $this->EE->db->query(
			"SELECT COUNT(*) AS count
			 ...
			 WHERE  group_id != '1'
			 AND 	group_id = '" .
				$this->EE->db->escape_str($this->insert_data['group_id']) . "'"
		);
		...
		if ($this->EE->config->item('secure_forms') == 'y')
		{
			...
			if (is_callable(array($this->EE->security, 'check_xid')))
			{
				$good = $this->EE->security->check_xid($this->EE->input->get_post('XID'));
			}
			else
			{
				$query = $this->EE->db->query(
			...
			if ( $remote === FALSE )
			{
				if (is_callable(array($this->EE->security, 'delete_xid')))
				{
					$this->EE->security->delete_xid($this->EE->input->get_post('XID'));
				}
				else
				{
					$this->EE->db->query(
		...
		if (APP_VER < '2.2.0')
		{
			$this->insert_data['password'] = $this->EE->functions->hash(stripslashes($_POST['password']));
		}
		else
		{
			$pass_data = $this->EE->auth->hash_password( stripslashes($_POST['password']));

			$this->insert_data['password']    	= $pass_data['password'];
			$this->insert_data['salt']    		= $pass_data['salt'];
		}

		$this->insert_data['ip_address']  = $this->EE->input->ip_address();
		$this->insert_data['unique_id']   = $this->EE->functions->random('encrypt');
		$this->insert_data['join_date']   = $this->EE->localize->now;
		...
		$this->insert_data['daylight_savings']	= (
			$this->EE->input->post('daylight_savings') == 'y'
		) ? 'y' : 'n';
		...
		{
			$this->insert_data['authcode'] = $this->EE->functions->random('alpha', 10);
		}
		...
		$this->EE->db->query($this->EE->db->insert_string('exp_members', $this->insert_data));
		...
		{
			$this->EE->session->userdata['language'] = $_POST['language'];
		}
			...
			foreach($lists as $list_id)
			{
				...
				$query = $this->EE->db->query(
					"SELECT list_title
					 ...
					 WHERE 	list_id = '" . $this->EE->db->escape_str($list_id) . "'"
				);
				...
				$results = $this->EE->db->query(
					"SELECT COUNT(*) AS count
					 ...
					 WHERE 	email = '" . $this->EE->db->escape_str($_POST['email']) . "'
					 AND 	list_id = '" . $this->EE->db->escape_str($list_id) . "'"
				);
				...
				if ($query->num_rows() > 0 AND $results->row('count') == 0)
				{
						...
						count($lists) == 1)
					{
						...
						$this->EE->db->query(
							"INSERT INTO exp_mailing_list_queue (email, list_id, authcode, date)
							 VALUES 	('".$this->EE->db->escape_str($_POST['email'])."', '" .
					...
					elseif ($this->EE->config->item('req_mbr_activation') == 'manual' OR
							$this->EE->config->item('req_mbr_activation') == 'email')
					{
					...
					else
					{
						...
						$this->EE->db->query(
							"INSERT INTO exp_mailing_list (user_id, list_id, authcode, email)
							 VALUES 	('', '".$this->EE->db->escape_str($list_id)."', '" .
										$code."', '" .
										$this->EE->db->escape_str($_POST['email'])."')"
						);
		...
		if ( ( $this->EE->config->item('new_member_notification') == 'y' AND
			   $this->EE->config->item('mbr_notification_emails') != '' ) OR
			   $notify != '' )
		{
			...
			$this->EE->email->from(
				$this->EE->config->item('webmaster_email'),
				$this->EE->config->item('webmaster_name')
			);
		...
		if ($this->EE->extensions->active_hook('user_register_end') === TRUE)
		{
			$edata = $this->EE->extensions->universal_call('user_register_end', $this, $member_id);
			if ($this->EE->extensions->end_script === TRUE) return;
		}
		...
		if ( $this->EE->config->item('req_mbr_activation') == 'email' )
		{
			...
			$action_id  = $this->EE->functions->fetch_action_id('User', 'activate_member');

			if (APP_VER < 2.0)
			{
				$action_id = $this->EE->functions->insert_action_ids($action_id);
			}
			...
			$forum_id = ($this->EE->input->get_post('FROM') == 'forum') ? '&r=f' : '';

			$add = ($mailinglist_subscribe !== TRUE) ? '' : '&mailinglist='.$list_id;

			$swap = array(
				'name'				=> $name,
				'activation_url'	=> $this->EE->functions->fetch_site_index(0, 0) .
										$qs . 'ACT='.$action_id .
			...
			$this->EE->email->to($this->insert_data['email']);
			$this->EE->email->subject($email_tit);
			$this->EE->email->message(entities_to_ascii($email_msg));
			$this->EE->email->Send();
		...
		elseif($this->_param('admin_register') != 'yes')
		{
			...
			if (APP_VER < '2.2.0')
			{
				...
				$this->EE->functions->set_cookie(
					$this->EE->session->c_expire ,
					time() + $expire,
					$expire
				);

				$this->EE->functions->set_cookie(
					$this->EE->session->c_uniqueid ,
					$this->insert_data['unique_id'],
					$expire
				);

				$this->EE->functions->set_cookie(
					$this->EE->session->c_password ,
					$this->insert_data['password'],
					$expire
				);
					...
					$this->EE->config->item('user_session_type') == 's')
				{
					...
					// $this->EE->functions->set_cookie($this->EE->session->c_session ,
						//$this->EE->session->sdata['session_id'],
				...
				$this->EE->session->userdata['username']		= $this->insert_data['username'];
				$this->EE->session->userdata['email']		= $this->insert_data['email'];
				$this->EE->session->userdata['screen_name']	= $this->insert_data['screen_name'];
				$this->EE->session->userdata['url']			= $this->insert_data['url'];
				$this->EE->session->userdata['location']		= $this->insert_data['location'];
				$this->EE->session->userdata['member_id']	= $member_id;
				$this->EE->session->userdata['group_id']		= $this->insert_data['group_id'];
					...
					$this->EE->config->item('disable_all_tracking') == 'n')
				{
					...
					$this->EE->db->query(
						"DELETE FROM exp_online_users
						 WHERE (ip_address = '".$this->EE->db->escape_str($this->EE->input->ip_address())."'
								AND member_id = '0')
						 OR 	date < $cutoff"
					);
			...
			{
				$member 	= $this->EE->db->get_where('members', array('member_id' => $member_id));
		...
		elseif ( $this->EE->config->item('req_mbr_activation') != 'email')
		{
	...
	{
		$qs			= ($this->EE->config->item('force_query_string') == 'y') ? '' : '?';
		$action_id	= $this->EE->functions->fetch_action_id('User', 'do_logout');
		...
		$return	= ( $this->EE->TMPL->fetch_param('return') !== FALSE AND $this->EE->TMPL->fetch_param('return') != '' ) ? $this->EE->TMPL->fetch_param('return'): $this->EE->uri->uri_string;
		...
		$tagdata	= $this->EE->functions->prep_conditionals( $this->EE->TMPL->tagdata, $swap );
	...
	public function do_logout ()
	{
		...
		*/
			$edata = $this->EE->extensions->call('user_logout');
			if ($this->EE->extensions->end_script === TRUE) return;
		/*
		/* -------------------------------------------*/
		...
		$return	= $this->EE->functions->create_url('');

		if ( $this->EE->input->get_post('return') !== FALSE )
		{
			$return	= urldecode( $this->EE->input->get_post('return') );
		}
		...
		$data = array(	'title' 	=> lang('mbr_login'),
						...
						'redirect'	=> $return,
						'link'		=> array($return, stripslashes($this->EE->config->item('site_name')))
					 );
	...
	private function mailing_list_email($list_title, $email, $code)
	{
		...
		$template = $this->EE->functions->fetch_email_template(
	...
	{
		if ($this->EE->config->item('mailinglist_notify') == 'y' AND
			$this->EE->config->item('mailinglist_notify_emails') != '')
		{
			$swap = array(
				'email'			=> $email,
				'mailing_list' 	=> $list_title
			...
			$notify_address = reduce_multiples(
				$this->EE->config->item('mailinglist_notify_emails'), ',', TRUE
			);
	...
	{
		$wquery = $this->EE->db->query("SELECT preference_name, preference_value FROM exp_user_preferences
		...
		$swap = array(LD.'site_name'.RD		=> $this->EE->config->item('site_name'),
					  LD.'site_url'.RD		=> $this->EE->config->item('site_url'),
		...
		$this->EE->db->query("DELETE FROM exp_user_welcome_email_list WHERE member_id = '".$this->EE->db->escape_str($row['member_id'])."'");
	...
	private function _member_update_email($old_data, $new_data, $emails, $message)
	{
		...
		if (preg_match_all("/".LD."changed(.*?)".RD."(.*?)".LD.'\/changed'.RD."/s", $message, $matches))
		{
			...
			foreach($dates as $date)
			{
				...
				{
					$new_data[$date] = $this->EE->localize->set_human_time($new_data[$date]);
		...
		$message = str_replace(LD.'update_date'.RD, $this->EE->localize->set_human_time($this->EE->localize->now), $message);

		/**	----------------------------------------
		/**	Parse dates
		/**	----------------------------------------*/

		foreach (array('update_date' => $this->EE->localize->now) as $key => $val)
		{
			if (preg_match("/".LD.$key."\s+format=[\"'](.*?)[\"']".RD."/s", $message, $match))
			{
				$str	= $match['1'];

				$codes	= $this->EE->localize->fetch_date_params( $match['1'] );

				foreach ( $codes as $code )
				{
					$str	= str_replace( $code, $this->EE->localize->convert_timestamp( $code, $val, TRUE ), $str );
				}
		...
		$this->EE->email->wordwrap = true;
		$this->EE->email->from($this->EE->config->item('webmaster_email'), $this->EE->config->item('webmaster_name'));
		$this->EE->email->to(reduce_multiples($emails, ',', TRUE));
		$this->EE->email->subject(lang('member_update'));
		$this->EE->email->message(entities_to_ascii($message));
		$this->EE->email->Send();
	...
	public function cp_validate_members($member_ids = array())
	{
		...
		$query = $this->EE->db->query("SELECT member_id, group_id, email, screen_name, username
							 FROM exp_members
							 WHERE member_id IN ('".implode("','", $this->EE->db->escape_str($member_ids))."')");
		...
		if ($this->EE->db->table_exists('exp_user_activation_group'))
		{
			$aquery = $this->EE->db->query("SELECT group_id, member_id FROM exp_user_activation_group
								  WHERE member_id IN ('".implode("','", $this->EE->db->escape_str($member_ids))."')
								  AND group_id != 0");
			...
			foreach($aquery->result_array() as $row)
			{
				$this->EE->db->query("UPDATE exp_members
							SET group_id = '".$this->EE->db->escape_str($row['group_id'])."'
							WHERE member_id = '".$this->EE->db->escape_str($row['member_id'])."'");
			}

			$this->EE->db->query("DELETE FROM exp_user_activation_group
						WHERE member_id IN ('".implode("','", $this->EE->db->escape_str($member_ids))."')");
		}
	...
	public function activate_member()
	{
		...
		$query = $this->EE->db->query(
			"SELECT member_id, group_id, email, screen_name, username
			 FROM 	exp_members
			 WHERE 	authcode = '".$this->EE->db->escape_str($id)."'"
		);
		...
		if ($this->EE->input->get_post('mailinglist') !== FALSE AND
			is_numeric($this->EE->input->get_post('mailinglist')))
		{
			...
			$results = $this->EE->db->query(
				"SELECT authcode
				 ...
				 AND 	list_id = '".$this->EE->db->escape_str(
					 $this->EE->input->get_post('mailinglist')
				 ) . "'"
			);
			...
			if ($results->num_rows() > 0)
			{
				...
				$this->EE->db->query(
					"INSERT INTO exp_mailing_list (user_id, list_id, authcode, email)
					 VALUES 	 ('', '" .
						$this->EE->db->escape_str($this->EE->input->get_post('mailinglist')) . "', '" .
						$this->EE->db->escape_str($results->row('authcode')) . "', '" .
						$this->EE->db->escape_str($query->row('email')) .
					 "')"
				);
				...
				//get title
				$title_query = $this->EE->db->query(
					"SELECT list_title
					 FROM 	exp_mailing_lists
					 WHERE 	list_id = '" . $this->EE->db->escape_str(
						 $this->EE->input->get_post('mailinglist')
					) . "'"
				);
		...
		{
			$edata = $this->EE->extensions->universal_call('member_register_validate_members', $member_id);
			if ($this->EE->extensions->end_script === TRUE) return;
		}
		//
		// -------------------------------------------
	...
	public function _remote_login()
	{
		...
		if ( ! $this->EE->input->get('multi') AND
			 ( ! $this->EE->input->post('username') OR ! $this->EE->input->post('password')))
		{
		...
		if ( $this->EE->input->get('multi') === FALSE )
		{
			...
			if ( $this->EE->config->item('site_id') !== FALSE )
			{
				$sql	.= " AND exp_member_groups.site_id = '".$this->EE->db->escape_str($this->EE->config->item('site_id'))."'";
			}
		...
		else
		{
			...
			if ($this->EE->input->get('cur') === FALSE OR $this->EE->input->get('orig') === FALSE)
			{
				return $this->_output_error('general', array(lang('not_authorized')));
			}
			...
			//
			if ($this->EE->extensions->active_hook('member_member_login_multi') === TRUE)
			{
				$edata = $this->EE->extensions->universal_call('member_member_login_multi', $query->row);
				if ($this->EE->extensions->end_script === TRUE) return;
			}
			//
			// -------------------------------------------
			...
			$next	= ($this->EE->input->get('cur') + 1 != $this->EE->input->get('orig')) ?
						$this->EE->input->get('cur') + 1 :
						$this->EE->input->get('cur') + 2;
			...
			else
			{
				...
				$next_url = $sites[$next].'?ACT='.$this->EE->functions->fetch_action_id('Member', 'member_login').
							'&multi='.$this->EE->input->get('multi').'&cur='.$next.'&orig='.$this->EE->input->get_post('orig');
		...
		if (APP_VER < '2.2.0')
		{
			...
			if ($query->row('password') != $password)
			{
				...
				$orig_enc_type = $this->EE->config->item('encryption_type');

				$this->EE->config->set_item(
					'encryption_type',
					($this->EE->config->item('encryption_type') == 'md5') ? 'sha1' : 'md5'
				);
		...
		{
			$passwd = $this->EE->auth->hash_password(stripslashes($this->EE->input->post('password')), $query->row('salt'));
		...
		{
			$member 	= $this->EE->db->get_where('members', array('member_id' => $query->row('member_id')));
		...
		$this->EE->session->userdata['screen_name']	= $query->row('screen_name');
		$this->EE->session->userdata['email']		= $query->row('email');
		$this->EE->session->userdata['url']			= $query->row('url');
		$this->EE->session->userdata['location']		= $query->row('location');
	...
	public function search()
	{
		...
		foreach ( $this->EE->uri->segments as $seg )
		{
			if ( strlen($seg) >= 32 )
			{
				$search_id	= $seg;
		...
		if ( $search_id != '' )
		{
			$query	= $this->EE->db->query( "SELECT `keywords`, `categories`, `fields`, `cfields` FROM exp_user_search WHERE search_id = '".$this->EE->db->escape_str( $search_id )."'" );
		...
		{
			$catq	= $this->EE->db->query( "SELECT cat_id, parent_id FROM exp_categories WHERE cat_id IN ('".implode( "','", $this->assigned_cats )."')" );
		...
		$this->form_data['ACT']						= $this->EE->functions->fetch_action_id('User', 'do_search');

		$this->form_data['RET']						= (isset($_POST['RET'])) ? $_POST['RET'] : $this->EE->functions->fetch_current_uri();

		$this->form_data['id']						= ( $this->EE->TMPL->fetch_param('form_id') ) ? $this->EE->TMPL->fetch_param('form_id'): '';

		$this->form_data['class']					= ( $this->EE->TMPL->fetch_param('form_class') ) ? $this->EE->TMPL->fetch_param('form_class'): '';

		$this->form_data['skip_field']				= ( $this->EE->TMPL->fetch_param('skip_field') ) ? $this->EE->TMPL->fetch_param('skip_field'): '';

		$this->form_data['group_id']					= ( $this->EE->TMPL->fetch_param('group_id') ) ? $this->EE->TMPL->fetch_param('group_id'): '';

		$this->form_data['return']					= ( $this->EE->TMPL->fetch_param('return') ) ? $this->EE->TMPL->fetch_param('return') : '';

		$this->form_data['inclusive_categories']		= ( $this->EE->TMPL->fetch_param('inclusive_categories') ) ? $this->EE->TMPL->fetch_param('inclusive_categories'): '';
	...
	public function do_search()
	{
		...
		if ( $this->EE->input->post('return') === FALSE OR $this->EE->input->post('return') == '' )
		{
			if ( $this->EE->input->post('RET') !== FALSE AND $this->EE->input->post('RET') != '' )
			{
				$return	= $this->EE->input->post('RET');
			}
			else
			{
		...
		if ($this->EE->session->userdata['search_flood_control'] > 0 AND $this->EE->session->userdata['group_id'] != 1)
		{
			...
			if ( $this->EE->config->item('site_id') !== FALSE )
			{
				$sql .= "site_id = '".$this->EE->db->escape_str($this->EE->config->item('site_id'))."' AND ";
			}
		...
		if ( $this->EE->input->post('group_id') !== FALSE AND $this->EE->input->post('group_id') != '' )
		{
		...
		if ( $this->EE->input->post('search_within_results') !== FALSE AND $this->EE->input->post('search_within_results') == 'yes' AND $this->_param('search_id') !== FALSE AND $this->_param('search_id') != '' )
		{
			$memq	= $this->EE->db->query("SELECT member_ids FROM exp_user_search WHERE search_id = '".$this->EE->db->escape_str( $this->_param('search_id') )."'");
		...
		if ( isset( $_POST['category'] ) !== FALSE )
		{
			...
			{
				$_POST['category']	= $this->EE->security->xss_clean( $_POST['category'] );
			...
			{
				$categories[]	= $this->EE->security->xss_clean( $_POST['category'] );
			}
		}
		...
		if ( ( $keywords = $this->EE->input->post('keywords') ) !== FALSE )
		{
		...
		if ( $this->EE->input->post('skip_field') !== FALSE AND $this->EE->input->post('skip_field') != '' )
		{
			$skip	= explode( "|", $this->EE->input->post('skip_field') );
		}
		...
		$this->insert_data	= $this->EE->security->xss_clean( $this->insert_data );
		...
		$cfields	= $this->EE->security->xss_clean( $cfields );
		...
		if ( count( $categories ) > 0 )
		{
			...
			{
				$sql	.= " ucp.cat_id = '".$this->EE->db->escape_str( $cat )."' OR";
			}
		...
		if ( count( $this->insert_data ) > 0 )
		{
			...
			foreach ( $this->insert_data as $key => $val )
			{
				if ( $compare == 'like' )
				{
					$sql	.= " m.".$key." LIKE '%".$this->EE->db->escape_str( $val )."%'".$andor;
				}
				else
				{
					$sql	.= " m.".$key." = '".$this->EE->db->escape_str( $val )."'".$andor;
				}
			}
		...
		if ( count( $cfields ) > 0 )
		{
			...
			foreach ( $cfields as $key => $val )
			{
				if ( $compare == 'like' )
				{
					$sql	.= " md.".$key." LIKE '%".$this->EE->db->escape_str( $val )."%'".$andor;
				}
				else
				{
					$sql	.= " md.".$key." = '".$this->EE->db->escape_str( $val )."'".$andor;
				}
			}
		...
		if ( count( $keywords ) > 0 )
		{
			...
			$keywords	= $this->EE->security->xss_clean( $keywords );
			...
			foreach ( $keywords as $keyword )
			{
				...
				foreach ( $this->standard as $val )
				{
					...
					$sql	.= " m.".$val." LIKE '%".$this->EE->db->escape_str( $keyword )."%'".$andor."\n";
				}
				...
				foreach ( $this->_mfields() as $key => $val )
				{
					...
					$sql	.= " md.m_field_id_".$val['id']." LIKE '%".$this->EE->db->escape_str( $keyword )."%'".$andor."\n";
				}
			}
		...
		if ( count( $exclude ) > 0 )
		{
			...
			$exclude	= $this->EE->security->xss_clean( $exclude );
			...
			foreach ( $exclude as $ex )
			{
				...
				foreach ( $this->standard as $val )
				{
					...
					$sql	.= " m.".$val." NOT LIKE '%".$this->EE->db->escape_str( $ex )."%'".$andor."\n";
				}
				...
				foreach ( $this->mfields as $key => $val )
				{
					...
					$sql	.= " md.m_field_id_".$val['id']." NOT LIKE '%".$this->EE->db->escape_str( $keyword )."%'".$andor."\n";
				}
			}
		...
		if ( count( $categories ) > 1 AND $this->EE->input->post('inclusive_categories') !== FALSE AND $this->EE->input->post('inclusive_categories') == 'yes' )
		{
			$chosen	= array();

			$catq	= $this->EE->db->query( "SELECT member_id, cat_id FROM exp_user_category_posts WHERE cat_id IN ('".implode( "','", $categories )."')" );
		...
		if ( $fail === FALSE )
		{
			...
			$sql	= "SELECT m.*, md.*, ( m.total_entries + m.total_comments ) AS total_combined_posts, mg.group_title, mg.group_description
					   ...
					   AND mg.site_id = '".$this->EE->db->escape_str($this->EE->config->item('site_id'))."'";
		...
		$data = array(
				...
				'search_date'	=> time(),
				'member_id'		=> $this->EE->session->userdata('member_id'),
				'keywords'		=> $this->EE->security->xss_clean( $this->EE->input->post('keywords') ),
				'ip_address'	=> $this->EE->input->ip_address(),
		...
		$this->EE->db->query( $this->EE->db->insert_string('exp_user_search', $data) );
	...
	public function results()
	{
		...
		$this->EE->db->query(
			"DELETE FROM 	exp_user_search
			 WHERE 			search_date < '" . $this->EE->db->escape_str($expire) . "'"
		);
	...
	public function forgot_username()
	{
		...
		$this->form_data['id']	= ( $this->EE->TMPL->fetch_param('form_id') ) ? $this->EE->TMPL->fetch_param('form_id'): 'forgot_username_form';

		$this->form_data['ACT']	= $this->EE->functions->fetch_action_id('User', 'retrieve_username');

		$this->form_data['RET']	= ( $this->EE->TMPL->fetch_param('return') != '' ) ? $this->EE->TMPL->fetch_param('return'): '';
	...
	public function retrieve_username()
	{
		...
		$sql = "SELECT member_id, username, screen_name, email, language FROM exp_members
				WHERE email ='".$this->EE->db->escape_str($address)."'";
		...
		{
			if ($this->EE->input->get_post('board_id') !== FALSE AND is_numeric($this->EE->input->get_post('board_id')))
			{
				$query	= $this->EE->db->query("SELECT board_forum_url, board_id, board_label
												FROM exp_forum_boards
												WHERE board_id = '".$this->EE->db->escape_str($this->EE->input->get_post('board_id'))."'");
			}
			else
			{
				$query	= $this->EE->db->query("SELECT board_forum_url, board_id, board_label
		...
		$wquery = $this->EE->db->query("SELECT preference_value FROM exp_user_preferences
		...
		$this->EE->email->subject(lang('forgotten_username_subject'));
		$this->EE->email->message($this->_var_swap(stripslashes($wquery->row('preference_value')), $swap));
	...
	public function forgot()
	{
		...
		$this->form_data['id']			= ( $this->EE->TMPL->fetch_param('form_id') ) ? $this->EE->TMPL->fetch_param('form_id'): 'forgot_password_form';

		$this->form_data['ACT']			= $this->EE->functions->fetch_action_id('User', 'retrieve_password');
	...
	public function key()
	{
		...
		if ( $this->EE->session->userdata('member_id') == 0 OR $this->EE->session->userdata('group_id') != 1)
		{
			return $this->_output_error('general', array(lang('not_authorized')));
		}
		...
		$this->form_data['ACT']				= $this->EE->functions->fetch_action_id('User', 'create_key');
		...
		$this->form_data['id']				= ( $this->EE->TMPL->fetch_param('form_id') ) ?
													$this->EE->TMPL->fetch_param('form_id') :
													'member_form';

		$this->params['template']			= ( $this->EE->TMPL->fetch_param('template') ) ? $this->EE->TMPL->fetch_param('template'): '';

		$this->params['html']				= $this->check_yes($this->EE->TMPL->fetch_param('html')) ? 'yes': 'no';

		$this->params['word_wrap']			= ( ! $this->check_yes($this->EE->TMPL->fetch_param('word_wrap'))) ? 'no': 'yes';

		$this->params['parse']				= ( in_array($this->EE->TMPL->fetch_param('parse'), array('br', 'none', 'xhtml'))) ?
												$this->EE->TMPL->fetch_param('parse') : 'none';

		$this->params['return']				= ( $this->EE->TMPL->fetch_param('return') ) ?
													$this->EE->TMPL->fetch_param('return') : '';

		$this->params['secure_action']		= ( ! $this->check_yes($this->EE->TMPL->fetch_param('secure_action'))) ? 'no': 'yes';
	...
	public function create_key()
	{
		...
		{
			if ($this->EE->input->get_post($var) !== FALSE)
			{
				$this->EE->load->helper('email');

				$email	= explode(",", $this->EE->input->get_post($var));
		...
		$this->insert_data['author_id']		= $this->EE->session->userdata['member_id'];
		$this->insert_data['date']			= $this->EE->localize->now;
		$this->insert_data['group_id']		= ( $this->EE->input->get_post('group_id') ) ?
												$this->EE->input->get_post('group_id'): '';
		...
		$vars['from_email']			= ( $this->EE->input->get_post('sender_email') !== FALSE ) ?
											$this->EE->input->get_post('sender_email') :
											(( $this->EE->input->get_post('from') !== FALSE ) ?
												$this->EE->input->get_post('from') :
												$this->EE->config->item('webmaster_email')
											);
		$vars['from_name']			= ( $this->EE->input->get_post('sender_name') !== FALSE ) ?
											$this->EE->input->get_post('sender_name') :
											(( $this->EE->input->get_post('name') !== FALSE ) ?
												$this->EE->input->get_post('name') :
												$this->EE->config->item('webmaster_name')
											);
		$vars['subject']			= ( $this->EE->input->get_post('subject') !== FALSE ) ?
											$this->EE->input->get_post('subject') :
											lang('you_are_invited_to_join') . ' ' .
												$this->EE->config->item('site_name');
		$vars['message']			= ( $this->EE->input->get_post('message') !== FALSE ) ?
											$this->EE->input->get_post('message') : '';
		...
		if ( $tmp	= $this->_param('template') )
		{
			...
			$query = $this->EE->db->query(
				"SELECT t.template_type, t.template_data
				 ...
				 WHERE 	tg.group_name = '" . $this->EE->db->escape_str($template['0']) . "'
				 AND 	t.template_name = '" . $this->EE->db->escape_str($template['1']) . "'
				 LIMIT 	1"
			);
			...
			$this->EE->template->encode_email	= FALSE;

			$this->EE->template->global_vars	= array_merge( $this->EE->template->global_vars, $vars );

			if (APP_VER >= 2.0)
			{
				$this->EE->config->_global_vars 	= array_merge( $this->EE->config->_global_vars, $vars );
			}

			$this->EE->template->run_template_engine( $template['0'], $template['1'] );
			...
			$body = $this->EE->typography->parse_type(
				stripslashes(
					//$this->EE->security->xss_clean(
						$this->EE->template->final_template
					//)
				),
				$formatting
			);
		...
		$to = $this->EE->input->get_post('to') ?
				$this->EE->input->get_post('to') :
				($this->EE->input->get_post('recipient_email') ?
					$this->EE->input->get_post('recipient_email') :
					FALSE
				);
		...
		if ( $to )
		{
			...
			foreach ( $email as $e )
			{
				...
				$this->insert_data['hash']	= $this->EE->functions->random( 'alpha' );
				...
				$this->EE->db->query( $this->EE->db->insert_string( 'exp_user_keys', $this->insert_data ) );
				...
				if ( $tmp	= $this->_param('template') )
				{
					...
					$this->EE->email->initialize();
					$this->EE->email->wordwrap	= ( $this->EE->input->get_post('word_wrap') == 'yes' ) ? true: false;
					$this->EE->email->mailtype	= ( $this->_param('html') == 'yes' ) ? 'html': 'text';
					$this->EE->email->from( $vars['from_email'], $vars['from_name'] );
					$this->EE->email->to( trim( $e ) );
					$this->EE->email->subject( $vars['subject'] );
					$this->EE->email->message( entities_to_ascii( $message ) );
					$this->EE->email->Send();
				}
			}
		...
		if ( $this->_param('return') !== FALSE AND $this->_param('return') != '' )
		{
			...
			$this->EE->functions->redirect( $this->EE->functions->create_url( str_replace( "%%key%%", implode( ",", $hashes ), $return ) ) );
		}
		else
		{
			// --------------------------------------------
			...
			$return	= ( $this->EE->input->get_post('return') ) ? $this->EE->input->get_post('return'): $this->EE->input->get_post('RET');
	...
	public function rank()
	{
		...
		if ( $this->EE->TMPL->fetch_param('member_id') == 'CURRENT_USER' AND $this->EE->session->userdata['member_id'] != '0' )
		{
			$sql	.= " AND author_id = '".$this->EE->session->userdata['member_id']."'";
		}
		...
		if ( $days = $this->EE->TMPL->fetch_param('days') )
		{
			$time	= $this->EE->localize->now - ( $days * 60 * 60 * 24);
		...
		if ( $query->num_rows() > 0 )
		{
			$i		= ( $this->EE->TMPL->fetch_param('entries_per_day') ) ? $this->EE->TMPL->fetch_param('entries_per_day'): 3;
		...
		if ( $this->EE->TMPL->fetch_param('member_id') == 'CURRENT_USER' AND $this->EE->session->userdata['member_id'] != '0' )
		{
			$sql	.= " AND c.author_id = '".$this->EE->session->userdata['member_id']."'";
		}
		...
		if ( $this->EE->TMPL->fetch_param('exclude') )
		{
			$sql	.= " AND c.author_id NOT IN (".$this->EE->TMPL->fetch_param('exclude').")";
		}
		...
		if ( $query->num_rows() > 0 )
		{
			$i		= ( $this->EE->TMPL->fetch_param('comments_per_day') ) ? $this->EE->TMPL->fetch_param('comments_per_day'): 3;
		...
		if ( $this->EE->db->table_exists('exp_favorites') )
		{
			/**	----------------------------------------
			...
			if ( $this->EE->TMPL->fetch_param('member_id') == 'CURRENT_USER' AND $this->EE->session->userdata['member_id'] != '0' )
			{
				$sql	.= " AND f.member_id = '".$this->EE->session->userdata['member_id']."'";
			}
			...
			if ( $this->EE->TMPL->fetch_param('exclude') )
			{
				$sql	.= " AND f.member_id NOT IN (".$this->EE->TMPL->fetch_param('exclude').")";
			}
			...
			if ( $query->num_rows() > 0 )
			{
				$i		= ( $this->EE->TMPL->fetch_param('favorites_per_day') ) ? $this->EE->TMPL->fetch_param('favorites_per_day'): 3;
		...
		if ( $this->EE->TMPL->fetch_param('member_id') )
		{
			...
			return $this->EE->TMPL->tagdata	= str_replace( LD.'total'.RD, $total, $this->EE->TMPL->tagdata );
		}
		...
		if ( ! $this->limit = $this->EE->TMPL->fetch_param('limit') )
		{
			$this->limit	= 50;
		}
		...
		$query	= $this->EE->db->query( "SELECT member_id, screen_name FROM exp_members
							   WHERE member_id IN (".$this->EE->db->escape_str( $mems ).")" );
	...
	public function inbox_count()
	{
		...
		$query	= $this->EE->db->query( "SELECT COUNT(*) AS count FROM exp_message_copies WHERE recipient_id = '".$this->EE->db->escape_str( $this->EE->session->userdata('member_id') )."' AND message_read = 'n' AND message_deleted = 'n' AND message_folder = '1'" );
	...
	public function delete_form()
	{
		...
		if ($this->member_id == $this->EE->session->userdata['member_id'] AND ($this->EE->session->userdata['can_delete_self'] !== 'y' OR $this->EE->session->userdata['group_id'] == 1))
		{
		...
		elseif($this->member_id != $this->EE->session->userdata['member_id'] AND $this->EE->session->userdata['group_id'] != 1 AND $this->EE->session->userdata['can_delete_members'] != 'y')
		{
			return '';
		}
		else
		{
			...
			$query = $this->EE->db->query("SELECT email, group_id, member_id, screen_name, username
								 FROM exp_members WHERE member_id = '".$this->EE->db->escape_str( $this->member_id )."'");
			...
			$this->EE->functions->prep_conditionals($tagdata, $query_row);
			...
			$this->form_data['id']	= ( $this->EE->TMPL->fetch_param('form_id') ) ? $this->EE->TMPL->fetch_param('form_id'): 'member_delete_form';

			$this->form_data['ACT']	= $this->EE->functions->fetch_action_id('User', 'delete_account');
	...
	public function delete_account()
	{
		...
		if ($this->_param('member_id') != $this->EE->session->userdata['member_id'])
		{
			if ($this->EE->session->userdata['group_id'] != 1 AND $this->EE->session->userdata['can_delete_members'] != 'y')
			{
				return $this->_output_error('general', lang('not_authorized'));
			}
		}
		elseif ($this->EE->session->userdata['can_delete_self'] !== 'y')
		{
			return $this->_output_error('general', lang('not_authorized'));
		}

		$admin = ($this->EE->session->userdata['member_id'] != $this->_param('member_id')) ? TRUE : FALSE;
		...
		$query = $this->EE->db->query(
			"SELECT m.*,
			 ...
			 WHERE 	m.member_id = '".$this->EE->db->escape_str($this->_param('member_id'))."'
			 AND 	m.group_id = mg.group_id"
		);
		...
		if ($this->EE->session->userdata['member_id'] != $this->_param('member_id'))
		{
			$squery = $this->EE->db->query(
				"SELECT password" . ((APP_VER < '2.2.0') ? '' : ', salt') . "
				 FROM 	exp_members
		...
		else
		{
			...
			$passwd = $this->EE->auth->hash_password(stripslashes($this->EE->input->post('password')), $check_salt);
		...
		if (APP_VER >= '2.4.0')
		{
			...
			$this->EE->member_model->delete_member($id);
		}
		else
		{
			/** -------------------------------------
			...
			$message_query = $this->EE->db->query(
			...
			if (APP_VER < 2.0)
			{
				$query = $this->EE->db->query("SELECT entry_id, weblog_id AS channel_id FROM exp_weblog_titles WHERE author_id = '{$id}'");
			}
			else
			{
			...
			if ($query->num_rows() > 0)
			{
				...
				if (APP_VER < 2.0)
				{
					$this->EE->db->query("DELETE FROM exp_weblog_titles WHERE author_id = '{$id}'");
					$this->EE->db->query("DELETE FROM exp_weblog_data WHERE entry_id IN ('".implode("','", $entry_ids)."')");
				}
				else
				{
				...
				$this->EE->db->query("DELETE FROM exp_trackbacks WHERE entry_id IN ('".implode("','", $entry_ids)."')");
			}

			// Find the affected entries AND channel ids for author's comments
			if (APP_VER < 2.0)
			{
				$query = $this->EE->db->query("SELECT DISTINCT(entry_id), weblog_id AS channel_id FROM exp_comments WHERE author_id = '{$id}'");
			}
			else
			{
			...
			if (count($recount_ids) > 0)
			{
				foreach (array_unique($recount_ids) as $entry_id)
				{
					...
					if (APP_VER < 2.0)
					{
						$this->EE->db->query("UPDATE exp_weblog_titles SET	comment_total = '".$this->EE->db->escape_str($query->row('count'))."',
																			recent_comment_date = '$comment_date' WHERE entry_id = '{$entry_id}'");
					}
					else
					{
						$this->EE->db->query("UPDATE exp_channel_titles SET comment_total = '".$this->EE->db->escape_str($query->row('count'))."',
																			recent_comment_date = '$comment_date' WHERE entry_id = '{$entry_id}'");
					}
				}
			}
			...
			foreach (array_unique($channel_ids) as $channel_id)
			{
				if (APP_VER < 2.0)
				{
					$this->EE->stats->update_weblog_stats($channel_id);
				}
				else
				{
		...
		if ($mbr_delete_notify_emails != '')
		{
			...
			$email_tit = $this->EE->functions->var_swap(lang('mbr_delete_notify_title'), $swap);
			$email_msg = $this->EE->functions->var_swap(lang('mbr_delete_notify_message'), $swap);
		...
		$this->EE->db->query("DELETE FROM exp_online_users
						  WHERE site_id = '".$this->EE->db->escape_str($this->EE->config->item('site_id'))."'
						  AND ip_address = '{$this->EE->input->ip_address()}'
						  AND member_id = '{$id}'");

		$this->EE->db->query("DELETE FROM exp_sessions WHERE member_id = '".$id."'");
		...
		if ($this->EE->extensions->active_hook('user_delete_account_end') === TRUE)
		{
			$edata = $this->EE->extensions->universal_call('user_delete_account_end', $this);
			if ($this->EE->extensions->end_script === TRUE) return;
		}
	...
	public function messages()
	{
		...
		if ( $this->EE->TMPL->fetch_param( 'folder' ) !== FALSE AND ctype_digit( $this->EE->TMPL->fetch_param( 'folder' ) ) )
		{
		...
		$sql	= "SELECT mc.copy_id AS message_id, md.message_subject, md.message_date FROM exp_message_copies mc LEFT JOIN exp_message_data md ON md.message_id = mc.message_id WHERE mc.recipient_id = '".$this->EE->db->escape_str( $this->EE->session->userdata('member_id') )."' AND mc.message_deleted = 'n' AND mc.message_folder = '".$this->EE->db->escape_str( $folder )."'";
		...
		if ( $this->EE->TMPL->fetch_param( 'limit' ) !== FALSE AND ctype_digit( $this->EE->TMPL->fetch_param( 'limit' ) ) )
		{
	...
	public function is_mine()
	{
		...
		{
			$this->EE->TMPL->log_item('Member ID: '.$this->member_id);
	...
	{
		return $this->EE->functions->redirect( $this->EE->config->item('cp_url')."?C=modules&M=user&P=reassign_ownership_confirm&member_id=".$this->EE->input->get_post('member_id')."&entry_id=".$this->EE->input->get_post('entry_id') );
	}
	...
	{
		if (APP_VER < 2.0 AND ! class_exists('Weblog'))
		{
			require PATH_MOD.'/weblog/mod.weblog'.EXT;
		}

		if ( APP_VER >= 2.0 AND ! class_exists('Channel'))
		{
	...
	public function _screen_name_override( $member_id = '0' )
	{
		...
		if ( ! empty($screen_name_override) )
		{
			...
			if ($this->EE->session->ban_check('screen_name', $screen_name))
			{
				return $this->_output_error(
					'general',
					array(
			...
			$sql	= "SELECT COUNT(*) AS count FROM exp_members WHERE screen_name = '".$this->EE->db->escape_str($screen_name)."'";
			...
			if ($update_member_data)
			{
				$this->EE->db->query(
					$this->EE->db->update_string(
						...
						'member_id = "' . $this->EE->db->escape_str($member_id) . '"'
					)
				);
			}
		}
	...
	public function _email_is_username( $member_id = '0', $type = 'update' )
	{
		...
		if ( $this->EE->config->item('allow_username_change') != 'y' AND $type == 'update' ) return FALSE;
		...
		$query	= $this->EE->db->query( "SELECT username, email FROM exp_members
									WHERE member_id = '".$this->EE->db->escape_str($member_id)."'" );
		...
		if ( $type == 'update' AND
			 ( ! $this->EE->input->get_post('username') OR
			  $this->EE->input->get_post('username') == $cur_username ) )
		{
			return TRUE;
		}
		...
		if ( $this->EE->session->ban_check( 'username', $this->EE->input->get_post('username') ) OR
			 $this->EE->session->ban_check( 'email', $this->EE->input->get_post('username') ) )
		{
		...
		$validate	= array(
			...
			'email'				=> $this->EE->input->get_post('username'),
			'cur_email'			=> $cur_email,
		);
		...
		$this->EE->load->library('validate', $validate, 'email_validate');

		$this->EE->email_validate->validate_username();

		if ($this->preferences['email_is_username'] != 'n' AND
			($key = array_search(
				lang('username_password_too_long'),
				$this->EE->email_validate->errors
			)) !== FALSE)
		{
			if (strlen($this->EE->email_validate->username) <= 50)
			{
				unset($this->EE->email_validate->errors[$key]);
			}
			else
			{
				$this->EE->email_validate->errors[$key] = str_replace('32', '50', $this->EE->email_validate->errors[$key]);
			}
		}
		...
		if( $this->preferences['email_is_username'] == 'y')
		{
			...
			{
				if (($key = array_search(lang($go_away), $this->EE->email_validate->errors)) !== FALSE)
				{
					unset($this->EE->email_validate->errors[$key]);
				}
			}
		...
		$this->EE->email_validate->validate_email();

		/**	----------------------------------------
		/**	Display errors if there are any
		/**	----------------------------------------*/

		if (count($this->EE->email_validate->errors) > 0)
		{
			return $this->_output_error('submission', $this->EE->email_validate->errors);
		}
		...
		$_POST['email']	= $this->EE->input->get_post('username');
	}
	...
	{
		$query = $this->EE->db->query("SELECT preference_value FROM exp_user_preferences WHERE preference_name = 'key_expiration' LIMIT 1");

		$exp	= ( $query->num_rows() > 0 ) ? $query->row('preference_value') : $exp;

		$now	= $this->EE->localize->now - ( $exp * 60 * 60 * 24 );

		$this->EE->db->query( "DELETE FROM exp_user_keys WHERE member_id = '0' AND date < '".$now."'" );
	...
	private function _update_last_activity()
	{
		...
		$this->EE->db->query( $this->EE->db->update_string( 'exp_members', array( 'last_activity' => $this->EE->localize->now ), array( 'member_id' => $member_id ) ) );
	...
	private function _remove_image($type, $member_id, $admin)
	{
		...
		if ($admin === FALSE AND ($this->check_yes($this->_param('password_required')) OR $this->_param('password_required') == 'yes'))
		{
			if ($this->EE->session->check_password_lockout() === TRUE)
			{
				$line = str_replace("%x", $this->EE->config->item('password_lockout_interval'), lang('password_lockout_in_effect'));
				return $this->_output_error('submission', $line);
			}
			...
			if (APP_VER < '2.2.0' AND
				$this->EE->functions->hash(stripslashes($_POST['current_password'])) != $query->row('password')
			)
			{
				return $this->_output_error( 'general', array( lang( 'invalid_password' ) ) );
			}
			...
			if (APP_VER >= '2.2.0' AND
				! $this->EE->auth->authenticate_id($member_id, $_POST['current_password']))
			{
				return $this->_output_error( 'general', array( lang( 'invalid_password' ) ) );
			}
	...
	private function _upload_image( $type = 'avatar', $member_id = 0, $test_mode = TRUE )
	{
		...
		if ( ! isset($_POST['remove']))
		{
			...
			if ($this->EE->config->item( $this->img['enable_pref'] ) == 'n')
			{
		...
		else
		{
			if ($type == 'avatar')
			{
				$query = $this->EE->db->query("SELECT avatar_filename FROM exp_members WHERE member_id = '".$member_id."'");
			...
			{
				$query = $this->EE->db->query("SELECT sig_img_filename FROM exp_members WHERE member_id = '".$member_id."'");
		...
		if ($max_width < $width OR $max_height < $height)
		{
			...
			else if (isset($this->EE->image_lib->error_msg) AND
					 count($this->EE->image_lib->error_msg) > 0)
			{
				return $this->EE->output->show_user_error(
					'submission',
					$this->EE->image_lib->display_errors()
				);
			}
		}
		...
		if ($type == 'avatar')
		{
			...
			$this->EE->db->query("UPDATE exp_members SET avatar_filename = '{$avatar}', avatar_width='{$width}', avatar_height='{$height}' WHERE member_id = '".$member_id."' ");
		}
		elseif ($type == 'photo')
		{
			$this->EE->db->query("UPDATE exp_members SET photo_filename = '{$new_filename}', photo_width='{$width}', photo_height='{$height}' WHERE member_id = '".$member_id."' ");
		}
		else
		{
			$this->EE->db->query("UPDATE exp_members SET sig_img_filename = '{$new_filename}', sig_img_width='{$width}', sig_img_height='{$height}' WHERE member_id = '".$member_id."' ");
		}
	...
	private function _image_resize($filename, $type = 'avatar', $axis = 'width')
	{
		if ($type == 'avatar')
		{
			$max_width	= (	$this->EE->config->slash_item('avatar_max_width') == '' OR
							$this->EE->config->item('avatar_max_width') == 0) ?
								100 : $this->EE->config->item('avatar_max_width');
			$max_height	= (	$this->EE->config->item('avatar_max_height') == '' OR
							$this->EE->config->item('avatar_max_height') == 0) ?
								100 : $this->EE->config->item('avatar_max_height');
			$image_path = rtrim( $this->EE->config->item('avatar_path'), '/' ) . '/' . 'uploads/';
		}
		elseif ($type == 'photo')
		{
			$max_width	= (	$this->EE->config->slash_item('photo_max_width') == '' OR
							$this->EE->config->item('photo_max_width') == 0) ?
								100 : $this->EE->config->item('photo_max_width');
			$max_height	= (	$this->EE->config->item('photo_max_height') == '' OR
							$this->EE->config->item('photo_max_height') == 0) ?
								100 : $this->EE->config->item('photo_max_height');
			$image_path = $this->EE->config->item('photo_path');
		}
		else
		{
			$max_width	= (	$this->EE->config->slash_item('sig_img_max_width') == '' OR
							$this->EE->config->item('sig_img_max_width') == 0) ?
								100 : $this->EE->config->item('sig_img_max_width');
			$max_height	= (	$this->EE->config->item('sig_img_max_height') == '' OR
							$this->EE->config->item('sig_img_max_height') == 0) ?
								100 : $this->EE->config->item('sig_img_max_height');
			$image_path = $this->EE->config->item('sig_img_path');
		}
		...
		$config = array(
			...
			'maintain_ratio'	=> TRUE,
			'master_dim'		=> $axis,
			'source_image'		=> $this->EE->functions->remove_double_slashes($image_path . '/' . $filename),
			'quality'			=> '75%',
			'width'				=> $max_width,
			'height'			=> $max_height
		);
	...
	{
		if (is_object($this->EE->TMPL) AND $this->check_no($this->EE->TMPL->fetch_param('log_views'))) return FALSE;

		if ( $this->member_id == 0 OR $this->member_id == $this->EE->session->userdata('member_id') ) return FALSE;

		// Only update once per page load
		if ( isset($this->cache[__FUNCTION__][$this->member_id])) return FALSE;

		$this->EE->db->set('profile_views', 'profile_views + 1', FALSE);
	...
	{
		if ( isset($this->EE->TMPL) AND is_object($this->EE->TMPL) AND $this->EE->TMPL->fetch_param('disable') !== FALSE AND stristr('member_data', $this->EE->TMPL->fetch_param('disable')))
		{
			return array();
		}
		...
		$query = $this->EE->db->query("SELECT m_field_id, m_field_name, m_field_label, m_field_type,
	...
	public function _member_id()
	{
		...
		$cat_segment	= $this->EE->config->item("reserved_category_word");
		...
		if ( ($member_id = $this->EE->TMPL->fetch_param('member_id')) !== FALSE)
		{
			...
			elseif($member_id == 'CURRENT_USER' AND $this->EE->session->userdata['member_id'] != 0)
			{
		...
		if ( ($member_id = $this->EE->TMPL->fetch_param('user_author_id')) !== FALSE)
		{
		...
		{
			if ($this->EE->TMPL->fetch_param('username') == 'CURRENT_USER' AND $this->EE->session->userdata['member_id'] != 0)
			{
				$this->member_id = $this->EE->session->userdata['member_id'];
			...
			$query	= $this->EE->db->query("SELECT member_id FROM exp_members
								  WHERE username = '".$this->EE->db->escape_str( $this->EE->TMPL->fetch_param('username') )."'");
		...
		if ( is_array($this->EE->TMPL->tagparams))
		{
			...
			if (count($search_fields) > 0)
			{
				...
				if ($sql != '')
				{
					$query = $this->EE->db->query("SELECT m.member_id
		...
		if ($dynamic === TRUE AND preg_match( "#/".self::$trigger."/(\w+)/?#", $this->EE->uri->uri_string, $match ) )
		{
			$sql	= "SELECT member_id FROM exp_members";

			if ( is_numeric( $match['1'] ) )
			{
				$sql	.= " WHERE member_id = '".$this->EE->db->escape_str( $match['1'] )."'";
			}
			else
			{
				$sql	.= " WHERE username = '".$this->EE->db->escape_str( $match['1'] )."'";
			}
		...
		if ( $this->EE->uri->uri_string != '' AND $dynamic === TRUE)
		{
			$qstring	= $this->EE->uri->uri_string;
			...
			else
			{
				...
				if (preg_match("#^".$cat_segment."/#", $qstring, $match) AND ($this->EE->TMPL->fetch_param('weblog') OR $this->EE->TMPL->fetch_param('channel')))
				{
					...
					if (APP_VER < 2.0)
					{
						...
						$xsql	= $this->EE->functions->sql_andor_string($this->EE->TMPL->fetch_param('weblog'), 'blog_name');
					}
					else
					{
					...
					if ($query->num_rows() == 1)
					{
						$result	= $this->EE->db->query("SELECT cat_id FROM exp_categories
														WHERE cat_name='".$this->EE->db->escape_str($qstring)."'
			...
			if ( $this->member_id != '' )
			{
				$query	= $this->EE->db->query( "SELECT member_id FROM exp_members WHERE member_id = '".$this->EE->db->escape_str( $this->member_id )."' LIMIT 1" );
		...
		foreach($search_fields as $field => $values)
		{
			...
			foreach($values as $value)
			{
				...
				elseif ( substr($value, 0, 1) == '%' OR substr($value, -1) == '%')
				{
					...
					{
						$parts[] = " ".$field." ".$like_comparision." '%".$this->EE->db->escape_like_str(substr($value, 1, -1))."%'";
					}
					elseif ( substr($value, 0, 1) == '%')
					{
						$parts[] = " ".$field." ".$like_comparision." '%".$this->EE->db->escape_like_str(substr($value, 1))."'";
					}
					else
					{
						$parts[] = " ".$field." ".$like_comparision." '".$this->EE->db->escape_like_str(substr($value, 0, -1))."%'";
					}
				}
				else
				{
					$parts[] = " ".$field." ".$comparision." '".$this->EE->db->escape_str($value)."'";
				}
			}
	...
	public function _entry_id()
	{
		...
		if ( ctype_digit( $this->EE->TMPL->fetch_param('entry_id') ) )
		{
			if (APP_VER < 2.0)
			{
				$query	= $this->EE->db->query( "SELECT entry_id FROM exp_weblog_titles
												 WHERE entry_id = '".$this->EE->db->escape_str( $this->EE->TMPL->fetch_param('entry_id') )."'" );
			}
			else
			{
				$query	= $this->EE->db->query( "SELECT entry_id FROM exp_channel_titles
												 WHERE entry_id = '".$this->EE->db->escape_str( $this->EE->TMPL->fetch_param('entry_id') )."'" );
			}
		...
		elseif ( $this->EE->uri->query_string != '' )
		{
			...
			if ( ctype_digit( $qstring ) )
			{
				...
				{
					$query	= $this->EE->db->query("SELECT entry_id FROM exp_weblog_titles
												WHERE entry_id = '".$this->EE->db->escape_str( $qstring )."'" );
				}
				else
				{
					$query	= $this->EE->db->query("SELECT entry_id FROM exp_channel_titles
			...
			else
			{
				...
				if (preg_match("#^".$cat_segment."/#", $qstring, $match) AND ($this->EE->TMPL->fetch_param('weblog') OR $this->EE->TMPL->fetch_param('channel')))
				{
					...
					if ( isset( $this->EE->TMPL->site_ids ) === TRUE )
					{
						$sql	.= " AND site_id IN ('".implode("','", $this->EE->TMPL->site_ids)."')";
					}

					if (APP_VER < 2.0)
					{
						$sql .= $this->EE->functions->sql_andor_string($this->EE->TMPL->fetch_param('weblog'), 'blog_name');
					}
					else
					{
					...
					if ($query->num_rows() == 1)
					{
						$sql	= "SELECT cat_id FROM exp_categories WHERE cat_name='".$this->EE->db->escape_str($qstring)."' AND group_id='{$query->row('cat_group')}'";

						if ( isset( $this->EE->TMPL->site_ids ) === TRUE )
						{
							$sql	.= " site_id IN ('".implode("','", $this->EE->TMPL->site_ids)."')";
						}
				...
				if (APP_VER < 2.0)
				{
					$sql	= "SELECT exp_weblog_titles.entry_id
								...
								AND exp_weblog_titles.url_title = '".$this->EE->db->escape_str($qstring)."'";

					if ( isset( $this->EE->TMPL->site_ids ) === TRUE )
					{
						$sql	.= " AND exp_weblog_titles.site_id IN ('".implode("','", $this->EE->TMPL->site_ids)."') ";
					}
				}
				else
				{
	...
	public function _parse_select_mailing_lists( $data, $row = array())
	{
		...
		if ( $data == '' OR $this->EE->db->table_exists( 'exp_mailing_lists' ) === FALSE)
		{
			return '';
		}
		...
		{
			$out		= $data;
			$out		= $this->EE->functions->prep_conditionals($out,$row);
	...
	public function _parse_select_member_groups( $data, $selected_group_id = 0)
	{
		...
		if ( $this->EE->TMPL->fetch_param('allowed_groups') === FALSE OR
			 $this->EE->TMPL->fetch_param('allowed_groups') == '')
		{
			return '';
		}
	...
	public function _parse_select( $key = '', $row = array(), $data = '' )
	{
		...
		foreach ( preg_split( "/\r|\n/", $this->mfields[$key]['list'] ) as $val )
		{
			...
			$out		= $this->EE->functions->prep_conditionals($out, array('value' => $val));
			$return		.= trim( $out )."\n";
		}
	...
	public function _form( $form_data = array() )
	{
		...
		if ($this->EE->TMPL->fetch_param('override_return') !== FALSE AND $this->EE->TMPL->fetch_param('override_return') != '')
		{
			$override_return = $this->EE->TMPL->fetch_param('override_return');

			if ( preg_match( "/".LD."\s*path=(.*?)".RD."/", $override_return, $match ) > 0 )
			{
				$override_return = $this->EE->functions->create_url( $match['1'] );
			}
			elseif ( stristr( $override_return, "http://" ) === FALSE AND
					 stristr( $override_return, "https://" ) === FALSE )
			{
				$override_return = $this->EE->functions->create_url( $override_return );
			}
		...
		foreach(array('return', 'RET') as $val)
		{
			if (isset($this->form_data[$val]) AND $this->form_data[$val] !== FALSE AND $this->form_data[$val] != '')
			{
				...
				{
					$this->form_data[$val] = $this->EE->functions->create_url( $match['1'] );
				}
				elseif ( stristr( $this->form_data[$val], "http://" ) === FALSE AND
						 stristr( $this->form_data[$val], "https://" ) === FALSE )
				{
					$this->form_data[$val] = $this->EE->functions->create_url( $this->form_data[$val] );
				}
			}
		}
		...
		if ($this->EE->TMPL->fetch_param('error_page') !== 'FALSE' AND
			$this->EE->TMPL->fetch_param('error_page') != '')
		{
			$arr['hidden_fields']['error_page'] = str_replace(T_SLASH, '/', $this->EE->TMPL->fetch_param('error_page'));
		}
	...
	public function _param( $which = '', $type = 'all' )
	{
		...
		if ( count( $this->params ) == 0 )
		{
			...
			if ( ! $this->params_id = $this->EE->input->get_post('params_id') )
			{
				return FALSE;
			}
			...
			$query	= $this->EE->db->query( "SELECT data FROM exp_user_params
								   WHERE hash = '".$this->EE->db->escape_str( $this->params_id )."'" );
		...
		if ( isset($this->EE->TMPL) AND is_object($this->EE->TMPL) AND $this->EE->TMPL->fetch_param($which) )
		{
	...
	private function _insert_params( $params = array() )
	{
		...
		$this->EE->db->query( $this->EE->db->insert_string( 'exp_user_params',
															array(	'hash' => $hash,
	...
	private function _order_sort( $sql , $additional = array())
	{
		...
		if ( $this->EE->TMPL->fetch_param('dynamic_parameters') !== FALSE &&
			 $this->EE->TMPL->fetch_param('dynamic_parameters') != 'no' &&
			 (
				( $this->EE->input->post('user_orderby') !== FALSE AND $this->EE->input->post('user_orderby') != '' )
				OR
				( $this->EE->input->cookie('user_orderby') !== FALSE AND $this->EE->input->cookie('user_orderby') != '' )
			 )
		   )
		{
			$this->_mfields();
			...
			elseif ( stristr( ",", $this->EE->input->post('user_orderby') ) )
			{
				$orderby	= explode( $this->EE->input->post('user_orderby') );
			}
			elseif ( stristr( ",", $this->EE->input->cookie('user_orderby') ) )
			{
				$orderby	= explode( $this->EE->input->cookie('user_orderby') );
			}
			else
			{
				$orderby	= ( $this->EE->input->post('user_orderby') !== FALSE ) ? array( $this->EE->input->post('user_orderby') ): array( $this->EE->input->cookie('user_orderby') );
			}
			...
			if ( count( $sort ) == 0 )
			{
				if ( $this->EE->input->post('user_sort') !== FALSE AND $this->EE->input->post('user_sort') != '' )
				{
					$sort	= ( $this->EE->input->post('user_sort') != 'asc' ) ? array('DESC'): array('ASC');
				}
				elseif ( $this->EE->input->cookie('user_sort') !== FALSE AND $this->EE->input->cookie('user_sort') != '' )
				{
					$sort	= ( $this->EE->input->cookie('user_sort') != 'asc' ) ? array('DESC'): array('ASC');
				}
				else
				{
					$sort	= array('ASC');
			...
			if ( $this->EE->input->post('user_orderby') !== FALSE )
			{
				$this->EE->functions->set_cookie( 'user_orderby', strtolower( implode( ",", $orderby ) ), 0 );
				$this->EE->functions->set_cookie( 'user_sort', strtolower( implode( ",", $sort ) ), 0 );
			}
		...
		elseif ( $this->EE->TMPL->fetch_param('orderby') !== FALSE AND $this->EE->TMPL->fetch_param('orderby') != '' )
		{
			...
			if ( stristr( $this->EE->TMPL->fetch_param('orderby'), "|" ) )
			{
				$orderby	= explode( "|", $this->EE->TMPL->fetch_param('orderby') );
			}
			else
			{
				$orderby	= array( $this->EE->TMPL->fetch_param('orderby') );
			}
			...
			if ( $this->EE->TMPL->fetch_param('sort') !== FALSE AND $this->EE->TMPL->fetch_param('sort') != '' )
			{
				if ( stristr( $this->EE->TMPL->fetch_param('sort'), "|" ) )
				{
					$sort	= explode( "|", strtoupper( $this->EE->TMPL->fetch_param('sort') ) );
				}
				else
				{
					$sort	= array( strtoupper( $this->EE->TMPL->fetch_param('sort') ) );
				}
			}
			else
			{
		...
		if ( $this->EE->TMPL->fetch_param('dynamic_parameters') !== FALSE AND $this->EE->TMPL->fetch_param('dynamic_parameters') != 'no' )
		{
			if ( $this->EE->input->post('user_limit') !== FALSE AND $this->EE->input->post('user_limit') != '' )
			{
				$this->limit	= $this->EE->input->post('user_limit');
				$this->EE->functions->set_cookie( 'limit', $this->EE->input->post('user_limit'), 0 );
			}
			elseif ( $this->EE->input->cookie('user_limit') !== FALSE AND $this->EE->input->cookie('user_limit') != '' )
			{
				$this->limit	= $this->EE->input->cookie('user_limit');
			}
		}
	...
	public function _prep_pagination( $sql, $url_suffix = '', $prefix = TRUE )
	{
		...
		$offset 		= ( $this->EE->TMPL->fetch_param('offset') OR ctype_digit($this->EE->TMPL->fetch_param('offset'))) ? $this->EE->TMPL->fetch_param('offset'): 0;

		$tsql	= preg_replace("/SELECT (DISTINCT)?(.*?)\s+FROM\s+/is", 'SELECT \\1 COUNT(*) AS count FROM ', $tsql);

		$query = $this->EE->db->query( $tsql );
	...
	private function _output_error($type, $errors)
	{
		...
		if ( REQ == 'PAGE' AND is_object($this->EE->TMPL) AND $this->EE->TMPL->fetch_param('error_page') !== FALSE)
		{
			$_POST['error_page'] = str_replace(T_SLASH, '/', $this->EE->TMPL->fetch_param('error_page'));
		}
		...
		$query = $this->EE->db->query(  "SELECT template_data, group_name, template_name, template_type
									FROM exp_templates AS t, exp_template_groups AS tg
									WHERE t.site_id = '".$this->EE->db->escape_str($this->EE->config->item('site_id'))."'
									AND t.group_id = tg.group_id
									AND t.template_name = '".$this->EE->db->escape_str($x[1])."'
									AND tg.group_name = '".$this->EE->db->escape_str($x[0])."'
									LIMIT 1");
		...
		if (is_array($data['link']) AND count($data['link']) > 0)
		{
			...
			$url = (strtolower($data['link']['0']) == 'javascript:history.go(-1)') ? $data['link']['0'] : $this->EE->security->xss_clean($data['link']['0']);
		...
		$this->EE->TMPL = $GLOBALS['TMPL'] = new Addon_builder_parser_user();
