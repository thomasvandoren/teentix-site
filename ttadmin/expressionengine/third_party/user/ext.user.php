...
class User_ext extends Extension_builder_user
{
	...
	public function __construct( $settings = '' )
	{
		...
		{
			$this->name			= lang('user_module_name');
			$this->description	= lang('user_module_description');
		}
	...
	public function loginreg( $data = array() )
	{
		if ( is_array( $this->EE->extensions->last_call ) &&
			count( $this->EE->extensions->last_call ) > 0 )
		{
			$data = $this->EE->extensions->last_call;
		}

		if ( $this->EE->input->post('user_login_type') === FALSE OR
			$this->EE->input->post('user_login_type') == '')
		{
			return $data;
		}
	...
	{
		if ( ! $this->EE->input->post('toggle') OR
