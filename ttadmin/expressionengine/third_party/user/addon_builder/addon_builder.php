...
class Addon_builder_user {
	...
	public function Addon_builder_user ($name='')
	{
		//path to this folder
		...
		if (APP_VER < 2.0)
		{
			...
			if (($parent_class == 'Extension_builder_user' OR
				 $super_parent_class == 'Extension_builder_user') AND
		...
		//--------------------------------------------
		// documentDOM_user instantiated, might move this.
		//--------------------------------------------

		if (REQ == 'CP' AND file_exists($this->aob_path . 'document_dom.php'))
		{
			if ( ! class_exists('documentDOM_user'))
			{
				require_once $this->aob_path . 'document_dom.php';
			}

			$this->document = new documentDOM_user();
		}
