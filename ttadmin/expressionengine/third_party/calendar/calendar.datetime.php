 ...
 * @package 	Solspace:Calendar
 * @author		Solspace DevTeam
 * @filesource 	./system/expressionengine/third_party/calendar/calendar.datetime.php
 */
...
class Calendar_datetime extends Calendar {
	...
	public function format_date_string($format, $prefix = '')
	{
		...
		if (! empty($matches))
		{
			foreach ($matches[1] as $k =>$char)
			{
				...
				{
					if (! $this->get_cached_format_ymd($this->ymd, $char))
					{
						$this->format_date_character($char);
					}
					$output = str_replace($matches[0][$k], $this->get_cached_format_ymd($this->ymd, $char), $output);
				}
				elseif (strpos($this->time_chars, $char) !== FALSE)
				{
					if (! $this->get_cached_time_format($this->time, $char))
					{
						$this->format_date_character($char);
					}
					$output = str_replace($matches[0][$k], $this->get_cached_time_format($this->time, $char), $output);
				}
			}
		}
	...
	public function format_date_character($format)
	{
		...
		switch($format)
		{
			...
			case 'b' :		// One letter day of the week - Proprietary
				$output = ee()->lang->line('day_'.$this->day_of_week.'_1');
				break;
			case 'd' :		// Day of the month, 2 digits
				$output = ($this->day >= 10) ? $this->day : '0'.$this->day;
				break;
			case 'D' :		// Three letter textual representation of the day
				$output = ee()->lang->line('day_'.$this->day_of_week.'_3');
				break;
			case 'j' :		// Day of the month, no leading zeros
				$output = $this->day;
				break;
			case 'l' :		// Fulltext day of the week
				$output = ee()->lang->line('day_'.$this->day_of_week.'_full');
				break;
			case 'N' :		// Numeric representation of day of week (1 = Monday, 7 = Sunday)
				$output = ($this->day_of_week == 0) ? 7 : $this->day_of_week; 
				break;
			case 'S' :		// Ordinal suffix for day of month, 2 characters
				// For numbers below 20 we use the ee()->lang value
				if ($this->day < 20)
				{
					$output = ee()->lang->line('suffix_' . $this->day);
				}
				// For numbers 20+ we look at the last digit
				else
				{
					$output = ee()->lang->line('suffix_' . substr($this->day, -1));
				}
				break;
			case 'w' :		// Numeric representation for day of the week (0 = Sunday)
				$output = $this->day_of_week;
				break;
			case 'W' :		// Week number of year, week starts on Monday
				$output = $this->get_week_number_W();
				break;
			case 'F' :		// Fulltext month
				$output = ee()->lang->line('month_'.$this->month.'_full');
				break;
			case 'm' :		// Month number, 2 characters
				$output = ($this->month >= 10) ? $this->month : '0'.$this->month;
				break;
			case 'M' :		// Three letter text representation of the month
			 	$output = ee()->lang->line('month_'.$this->month.'_3');
			 	break;
			case 'n' :		// Month number, no leading zeros
				$output = $this->month;
				break;
			...
			case 'U' :		// Unix timestamp
				$output = mktime($this->hour, $this->minute, 0, $this->month, $this->day, $this->year);
				break;
			case 'Y' :		// Year
				$output = $this->year;
				break;
			case 'y' :		// 2-digit year
				$output = ($this->year >= 10) ? substr($this->year, -2) : '0'.$this->year;
				break;
			case 'z' :		// Day of the year, 0 to 365
			...
			case 'a' :		// am/pm
				$output = ($this->pm === TRUE) ? ee()->lang->line('pm') : ee()->lang->line('am');
				break;
			case 'A' :		// AM/PM
				$output = ($this->pm === TRUE) ? ee()->lang->line('PM') : ee()->lang->line('AM');
				break;
			case 'g' :		// Hour, 12-hour format, no leading zeros
			...
			case 'h' :		
				...
				break;
			case 'H' :		// Hour, 24-hour format, leading zero
				if ($this->hour < 10)
				{
					$output = str_pad($this->hour, 2, '0', STR_PAD_LEFT);
				}
			...
			case 'i' :		// Minutes
				...
				break;
			case 'I' :		//	1 if DST, 0 if not
				$output = ($this->is_dst === TRUE) ? 1 : 0;
				break;
			case 's' :		// Seconds
				$output = '00';
				break;
		...
		if ($type == 'time')
		{
			$this->set_cached_date_format($this->time, $format, $output);
		}
		else
		{
			$this->set_cached_date_format($this->ymd, $format, $output);
		}
