 ...
 * @package 	Solspace:Calendar
 * @author		Solspace DevTeam
 * @filesource 	./system/expressionengine/third_party/calendar/calendar.event.php
 */
...
class Calendar_event extends Calendar {
	...
	{		
		...
		// -------------------------------------
		//  Use the start data if the equivalent end data doesn't exist and the event doesn't recur
		// -------------------------------------
		...
		$this->default_data['all_day'] = (isset($this->default_data['all_day']) AND 
										  $this->default_data['all_day'] == 'y') ? TRUE : FALSE;
		
		// -------------------------------------
		//  If the event is all day, the end time will be 0000. While calculating
		//  event duration we'll come up a whole day short as a result. Thus,
	...
	{
		$this->load_data();
		$this->occurrences = Calendar_data::fetch_occurrences_by_event_id($this->default_data['event_id'], $this->range_start, $this->range_end);
	}
	...
	public function add_dates_using_rule($rule)
	{
		...
		$e_ymd				= $rule['end_date'];
		$all_day			= (isset($rule['all_day']) AND $rule['all_day'] == 'y') ? TRUE : FALSE;
		...
		if ($s_ymd < $rule['end_date'])
		{
			...
			$this->CDT->change_date($s_year, $s_month, $s_day);
		}
		...
		{							
			//get the difference 
			$diff 		= $this->calculate_duration($this->CDT->datetime_array(), $diff_start_date) ;
		
			//calculation includes current day :/
			$diff['days'] = $diff['days'] - 1;
		
			$diff_days 	= ($diff['days'] - ($diff['days'] % ($rule['repeat_weeks'] * 7)) - ($rule['repeat_weeks'] * 7));
		...
		{				
			...
			$diff_days 	= ($diff['days'] - ($diff['days'] % $rule['repeat_days'])) - $rule['repeat_days'];
		...
		while ($this->CDT->ymd <= $rule['last_date'] 		AND 
			   ...
			   //this should be <, but it produces too few results at times :(
			   //however, this produces too many other times. Removed extra at bottom
			   $count <= $limit 
		)
		{
			//$count++;
			...
			{	
				...
				if ($add_it === TRUE)
				{
					...
					// ) ? FALSE : TRUE;
					$dates[$date['ymd']][$time]['multi_day'] = (
						$dates[$date['ymd']][$time]['date']['ymd'] == $dates[$date['ymd']][$time]['end_date']['ymd'] OR
	...
	protected function load_data()
	{
		...
		{
			require_once CALENDAR_PATH.'data.calendar'.EXT;
		}
	}
	/* END load_data() */
