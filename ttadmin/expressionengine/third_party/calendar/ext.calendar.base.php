...
class Calendar_extension_base extends Extension_builder_calendar
{
	...
	public function __construct($settings = array())
    {
	   //$this->theme = 'flow_ui';
    	...
    	parent::Extension_builder_calendar('calendar');
        ...
        if (is_object(ee()->lang))
        {
        	...
        	$this->name			= ee()->lang->line('calendar_module_name');
        	$this->description	= ee()->lang->line('calendar_module_description');
        }
	...
	{								
		...
		elseif ($this->data->channel_is_events_channel($data[$channel_id]) === TRUE)
		{
			...
			// -------------------------------------
			//  Verify that the user can add events to this calendar
			// -------------------------------------
			...
			if ( ! empty($errors))
			{
				...
				{
					$message .= '<p>' . ee()->lang->line($error) . '</p>';
				}

				return ee()->output->show_user_error('general', $message);
			}
				...
				foreach ($rules_data as $k => $rule)
				{
					...
					else
					{
						$info = array(	
							...
							'end_time'			=> $rule['end_time'],
							'repeat_years'		=> $rule['repeat_years'],
							...
							'stop_by'			=> $rule['stop_by'],
							'last_date'			=> $rule['last_date']
						);

						if ($edit === TRUE AND $info['rule_id'] != FALSE)
						{
				...
				if (ee()->calendar_permissions->enabled())
				{
					...
					$output .= $this->view(
						...
						array(
							...
							'lang_allow_all'		=> ee()->lang->line('allow_all'),
							'lang_permissions_instructions'	=> ee()->lang->line('permissions_instructions'),
							'lang_group_permissions' => ee()->lang->line('group_permissions'),
							'lang_instructions'		=> ee()->lang->line('instructions')
						),
						TRUE
					);
				}
		...
		for ($i = 0; $i < $number_of_rules; $i++)
		{
			...
			if ($occurrence === TRUE)
			{
				...
				if ($this->CDT->is_valid_ymd($data['start_date'][$i]) === TRUE)
				{
					...
					$rule_data['repeat_years']	= $rule_data['repeat_months'] = $rule_data['repeat_days'] = $rule_data['repeat_weeks'] = $rule_data['days_of_week'] = $rule_data['relative_dow'] = $rule_data['days_of_month'] = $rule_data['months_of_year'] = $rule_data['stop_after'] = $rule_data['stop_by'] = $rule_data['last_date'] = '';
					$rule_data['single_date']	= 'y'; 
    ...
    {    
		...
		// -------------------------------------
		//  I would prefer that this code never 
		//	had to be written, but it is a 
		//	workaround for a bug in addons/Addons_installer.php 
		//	on line 368 EE 2.1.0.
		// -------------------------------------
		
		if ( APP_VER >= 2.0 )
		{
			$sql	= "SELECT 		exp_channel_fields.field_id, exp_channels.channel_id
					   FROM 		exp_channel_fields
					   LEFT JOIN 	exp_channels 
					   ON 			exp_channels.field_group = exp_channel_fields.group_id
					   WHERE 		exp_channel_fields.field_type = 'calendar'";
			...
			if (count($ids))
			{
				...
				$this->EE->layout->delete_layout_fields($id, $channel_ids);				
	...
	public function cp_js_end ()
	{
		...
		if ( $group_id != 1)
		{
			$js .= $this->view('cp_js_end.js', array(
				...
				'lang_dialog_title' 	=> ee()->lang->line('permission_dialog_title'),
				'lang_dialog_message' 	=> ee()->lang->line('invalid_calendar_permissions'),
				'lang_ok' 		=> ee()->lang->line('ok')
			), TRUE);
		}
