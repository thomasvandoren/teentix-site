...
//ee()->TMPL->log_item('Calendar: No calendars, bailing');
		...
		ee()->TMPL->tagparams['weblog'] = CALENDAR_CALENDARS_CHANNEL_NAME;
...
//ee()->TMPL->log_item('Calendar: Processing parameters');
	...
	//ee()->TMPL->log_item('Calendar: Channel module says no results, bailing');
			...
			{
				$channel->query->result	= $channel->query->result;
				$super_temp_fake	= $channel->query->result[0];
			}
			else
			{
	...
	*/
	...
	public function ics_update()
	{
		...
		if ($this->P->value('time_range_start', 'time') !== FALSE AND $this->P->value('time_range_start', 'time') > $time)
		{
			//ee()->TMPL->log_item('Calendar: It is not time to update icalendar data yet.');
			return;
		}
		elseif ($this->P->value('time_range_end', 'time') !== FALSE AND $this->P->value('time_range_end', 'time') < $time)
		{
		...
		{
			$ids = $this->data->get_calendars_needing_update($ids, $this->P->value('minute_interval'));
		}
		...
		else
		{
			...
			if ($each_event != '' OR strpos(ee()->TMPL->tagdata, '_event_total') !== FALSE)
			{
				while (TRUE)
				{
					...
					//ee()->TMPL->tagdata = ee()->TMPL->assign_relationship_data( ee()->TMPL->tagdata );
					ee()->TMPL->tagdata 	= ee()->TMPL->assign_relationship_data( $each_event );
					ee()->TMPL->var_single 	= array_merge( ee()->TMPL->var_single, ee()->TMPL->related_markers );
