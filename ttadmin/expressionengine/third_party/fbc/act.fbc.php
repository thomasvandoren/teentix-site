 ...
 * @package 	Solspace:FBC
 * @author		Solspace DevTeam
 * @filesource 	./system/expressionengine/third_party/fbc/act.fbc.php
 */
...
class Fbc_actions extends Addon_builder_fbc
{
	...
	function Fbc_actions()
    {
    	parent::Addon_builder_fbc('fbc');
	...
	function _check_form_hash()
	{
        ...
        {
            $query = ee()->db->query("SELECT COUNT(*) AS count FROM exp_security_hashes WHERE hash='".ee()->db->escape_str($_POST['XID'])."' AND ip_address = '".ee()->db->escape_str(ee()->input->ip_address())."' AND date > UNIX_TIMESTAMP()-7200");
        
            if ($query->row('count') == 0)
            {
				return $this->error[]	= ee()->lang->line('not_authorized');
            }
                                
			ee()->db->query("DELETE FROM exp_security_hashes WHERE (hash='".ee()->db->escape_str($_POST['XID'])."' AND ip_address = '".ee()->db->escape_str(ee()->input->ip_address())."') OR date < UNIX_TIMESTAMP()-7200");
        }
	...
	function create_member_account( $uid = '', $member_data = array() )
	{
    	// --------------------------------------------
		//	Validate
		// --------------------------------------------
	...
	function ee_login( $member_id = '' )
	{
		...
		if (APP_VER >= '2.2.0')
		{
			...
			{
				$this->error[]	= ee()->lang->line('not_authorized');
				return FALSE;
			}
		}
        ...
        { 
			ee()->output->show_user_error('general', array(ee()->lang->line('mbr_account_not_active')));        
        }
        ...
        {			
			...
			ee()->functions->set_cookie( ee()->session->c_uniqueid , $member_data['unique_id'], $expire );
			
			if ( APP_VER < '2.2.0' )
			{
				ee()->functions->set_cookie( ee()->session->c_password , $member_data['password'], $expire );
			}        
			...
			ee()->session->create_new_session( $member_data['member_id'] );
			ee()->session->userdata['username']  	= $member_data['username'];
			ee()->session->userdata['screen_name']  = $member_data['username'];
			ee()->session->userdata['email']  		= $member_data['email'];
	...
	function log_to_cp( $msg = '', $member_data = array() )
	{
		return FALSE;
	...
	function passive_registration( $uid = '' )
	{
        // --------------------------------------------
	...
	function send_admin_notification_of_registration( $member_data = array() )
	{
	...
	function send_user_activation_email( $member_data = array() )
	{
