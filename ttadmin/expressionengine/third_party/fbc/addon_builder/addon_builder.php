...
class Addon_builder_fbc {
	...
	public function Addon_builder_fbc ($name='')
	{
		//path to this folder
		...
		if (APP_VER < 2.0)
		{
			...
			if (($parent_class == 'Extension_builder_fbc' OR 
				 $super_parent_class == 'Extension_builder_fbc') AND 
		...
		//--------------------------------------------
		// documentDOM_fbc instantiated, might move this.
		//--------------------------------------------

		if (REQ == 'CP' AND file_exists($this->aob_path . 'document_dom.php'))
		{
			if ( ! class_exists('documentDOM_fbc'))
			{
				require_once $this->aob_path . 'document_dom.php';
			}
		
			$this->document = new documentDOM_fbc();
		}
