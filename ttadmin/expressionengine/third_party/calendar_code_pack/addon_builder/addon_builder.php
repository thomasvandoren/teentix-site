...
class Addon_builder_calendar_code_pack {
	...
	public function Addon_builder_calendar_code_pack ($name='')
	{
		//path to this folder
		...
		if (APP_VER < 2.0)
		{
			...
			if (($parent_class == 'Extension_builder_calendar_code_pack' OR 
				 $super_parent_class == 'Extension_builder_calendar_code_pack') AND 
		...
		{			
			...
			{
				if ( ! class_exists('Addon_builder_data_calendar_code_pack'))
				{
					require_once $this->aob_path . '/data.addon_builder.php';
				}
				
				$this->data = new Addon_builder_data_calendar_code_pack($this);
			}
		}
		...
		//--------------------------------------------
		// documentDOM_calendar_code_pack instantiated, might move this.
		//--------------------------------------------

		if (REQ == 'CP' AND file_exists($this->aob_path . '/document_dom.php'))
		{
			if ( ! class_exists('documentDOM_calendar_code_pack'))
			{
				require_once $this->aob_path . '/document_dom.php';
			}
		
			$this->document = new documentDOM_calendar_code_pack();
		}
	...
	// END Addon_builder_calendar_code_pack()
...
EOT;
	...
	{
		$tmpl_on = (isset(ee()->TMPL) AND is_object(ee()->TMPL));

		if (($tmpl_on AND 
			ee()->TMPL->fetch_param('ajax_request') !== FALSE AND 
			$this->check_no(ee()->TMPL->fetch_param('ajax_request'))) OR
			(ee()->input->get_post('ajax_request') !== FALSE AND 
		...
		if (($tmpl_on AND 
			ee()->TMPL->fetch_param('ajax_request') !== FALSE AND 
			$this->check_yes(ee()->TMPL->fetch_param('ajax_request'))) OR 
			$this->check_yes(ee()->input->get_post('ajax_request'))
		)
		{
			return TRUE;
		}
