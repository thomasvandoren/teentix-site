...
if ( ! class_exists('Addon_builder_calendar_code_pack'))
{
	require_once 'addon_builder.php';	
}

class Addon_builder_data_calendar_code_pack {
	...
	function Addon_builder_data_calendar_code_pack(&$parent_aob_instance = FALSE)
    {	
    ...
    /* END Addon_builder_data_calendar_code_pack() */
	...
	{			
		...
		if ( ! in_array($method, $this->aob_non_static_friendly) AND 
			 is_callable(array('Addon_builder_calendar_code_pack', $method))) 
		{						
			return call_user_func_array(array('Addon_builder_calendar_code_pack', $method), $args);
		}
	}
 	//end __call
