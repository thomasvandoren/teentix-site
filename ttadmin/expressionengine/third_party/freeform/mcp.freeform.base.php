...
class Freeform_cp_base extends Module_builder_freeform 
{
	...
	function manage_entries($message='')
    {
		...
		if( ee()->input->get_post('toggle_fields') !== FALSE ) 
		{
			if( is_array( ee()->input->get_post('toggle_fields') ) )
			{
				foreach( ee()->input->get_post('toggle_fields') as $field )
				{
					$toggle_fields[] = ee()->db->escape_str( $field );
				}	
			}
			else
			{
				$toggle_fields = explode( '|', ee()->db->escape_str( ee()->input->get_post('toggle_fields') ));
			}

			$toggle_fields_all = FALSE;		
		}
		...
		if ( $cquery->row('count') == 0 )
		{
			...
			$this->cached_vars['fields_all']		= $fields_all;
			$this->cached_vars['toggle_fields'] 	= $toggle_fields;
			$this->cached_vars['toggle_fields_all'] = $toggle_fields_all;
		...
		//--------------------------------------
		//  Add our default field columns first
		//--------------------------------------

		$fields_all['edit'] 			= 'Edit';
		$fields_all['attachments'] 		= 'Attachments';
		$fields_all['status'] 			= 'Status';
		$fields_all['date'] 			= 'Date';
		$fields_all['form_name']	 	= 'Collection Name';
		$fields_all['template'] 		= 'Template';
		...
		if ( $cquery->row('count') > $row_limit )
		{
			...
			if( ee()->input->get_post('toggle_fields') )
			{
				$pageination_base_url	.= AMP . 'toggle_fields=' . implode( '|', $toggle_fields ) ;
			}
			
			if( ee()->input->get_post('show_empties') )
			{
				$pageination_base_url	.= AMP . 'show_empties=' . ee()->input->get_post('show_empties');
			}

			$input_count 	= ee()->input->get_post('row', TRUE);

			$row_count		= (is_numeric($input_count) AND $input_count >= 0) ? $input_count : 0;
		...
		if ( $qfields->num_rows() > 0 )
		{
			...
			{
				$fields_all[ $field['name'] ] = $field['label'];

				if ( $do_show_empties OR isset( $filled[ $field['name'] ] ) )
				{
					if( $toggle_fields_all === TRUE OR in_array( $field['name'], $toggle_fields ) )
					{
	...
	function edit_entry_form()
    {
		...
		// Tries to find fields that are always empty for this form
		if ( TRUE )
		{
			$sql	= "SELECT 	* 
					   FROM 	exp_freeform_entries 
		...
		$this->cached_vars['filled'] 			= $filled;
		$this->cached_vars['not_filled'] 		= $not_filled;
		$this->cached_vars['fields'] 			= $fields;
    ...
    {		
		...
		if ( ee()->input->get_post('template_id', TRUE) )
		{
			$edit	= TRUE;
	...
	private function _csv_prep($item, $is_csv, $comma = TRUE)
	{
		...
		if( $is_csv === TRUE )
		{
			...
			if( strpos( $item, ',') > 0 OR strpos( $item, '"' ) > 0 )
			{
    ...
    function edit_field()
    {
		...
		{
			$sql .= " AND field_id != '".ee()->db->escape_str(ee()->input->get_post('field_id'))."'";
		}
		...
		{		
			...
			$suffix = ( $field_type != 'text' ? " NOT NULL DEFAULT ''" : ' NOT NULL ' );
