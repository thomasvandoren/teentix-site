...
class Rating_code_pack_extension_base extends Extension_builder_rating_code_pack
{	
	var $name			= '';
	var $version		= '';
	var $description	= '';
	...
	function Rating_code_pack_extension_base( $settings = array() )
    {    	
    	parent::Extension_builder_rating_code_pack('rating_code_pack');
        ...
        {
        	ee()->lang->loadfile('rating_code_pack');
        
        	$this->name			= ee()->lang->line('rating_code_pack_name');
        	$this->description	= ee()->lang->line('rating_code_pack_description');
        }
        
		$this->version		= RATING_CODE_PACK_VERSION;
		$this->docs_url		= RATING_CODE_PACK_DOCS_URL;
	 ...
	 * Code pack install
	 *
	 * This method runs during the installation of a code pack in the Rating module CP. It adds some members, weblog data and some rating module data.
	 *
	 * @access	public
	 * @param	message
	 * @return	string
    ...
    function code_pack_install( &$ths, $variables )
    {
		...
		$members[]	= array(
			...
			'ratingname'		=> $variables['prefix'] . 'jake',
		...
		$members[]	= array(
			'group_id'		=> $member_group_id,
			'ratingname'		=> $variables['prefix'].'mike',
		...
		$sql	= "SELECT email, ratingname
			FROM exp_members
			WHERE ratingname IN ('" . implode( "','", $ratingnames ) . "')
			OR email IN ('" . implode( "','", $emails ) . "')";
		
		$query	= ee()->db->query( $sql );
		
		$conf_emails	= array();
		$conf_ratingnames	= array();
		...
		if ( $query->num_rows() > 0 )
		{
			foreach ( $query->result_array() as $row )
			{
				...
				$conf_ratingnames[]	= $row['ratingname'];
			}
		}
		...
		if ( count( $conf_ratingnames ) > 0 )
		{
			$arr	= array(
				'label'			=> ee()->lang->line('conflicting_ratingnames'),
				'description'	=> str_replace( '%conflicting_ratingnames%', '<div class="defaultBold">'.implode( ', ', $conf_ratingnames ).'</div>', ee()->lang->line('conflicting_ratingnames_exp') )
			);
    ...
    function code_pack_list( $code_packs )
    {
    	...
    	$this_code_pack['rating_code_pack']	= array(
    		'code_pack_name'			=> $this->lower_name,
    		'code_pack_label'			=> ee()->lang->line( 'rating_code_pack_name' ),
    		'code_pack_description'		=> ee()->lang->line( 'rating_code_pack_description' ),
    		'code_pack_theme_folder'	=> 'rating',
    	);
